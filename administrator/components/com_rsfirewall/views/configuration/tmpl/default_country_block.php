...
if (isset($this->fieldset->description) && !empty($this->fieldset->description)) { ?>
	...
	<?php if (!$this->geoip->mmdb) { ?>
		<div class="alert alert-info">
			...
			<div><button type="button" class="btn btn-primary" onclick="RSFirewall.GeoIPDownload(this)"><i class="icon-refresh"></i> <?php echo JText::_('COM_RSFIREWALL_DOWNLOAD_GEOIP_DB_LITE'); ?></button></div>
		</div>
	<?php } elseif (!empty($this->geoip->mmdb_old)) { ?>
		<div class="alert alert-info">
			...
			<div><button type="button" class="btn btn-primary" onclick="RSFirewall.GeoIPDownload(this)"><i class="icon-refresh"></i> <?php echo JText::_('COM_RSFIREWALL_UPDATE_GEOIP_DB_LITE'); ?></button></div>
		</div>
	<?php } ?>
<?php } ?>
...
foreach ($this->fields as $field)
{
	...
	if (!$this->geoip->works)
	{
		echo str_replace('type="checkbox"', 'type="checkbox" disabled', $this->form->renderField($field->fieldname));
	}
	else
	{
...
?>
<p><small><?php echo JText::_('COM_RSFIREWALL_MAXMIND_ATTRIBUTION_MESSAGE'); ?></small></p>
