...
class crawlerHelper {	
	...
	public function links() {
		...
		foreach ($dom->find('a') as $href) {
			...
			// Skip unwanted links
			if (substr($href,0,7) == 'mailto:' || substr($href,0,11) == 'javascript:' || substr($href,0,6) == 'ymsgr:' || substr($href,0,1) == '#' || substr($href,0,4) == 'tel:' || substr($href,0,6) == 'skype:' || substr($href,0,9) == 'facetime:' || empty($href)) {
				continue;
			}
