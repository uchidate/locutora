...
class WebpageSize {
    ...
    */
	protected function setBaseURL($str) {
		preg_match("/base.*[\s]*href[\040]*=[\040]*\"?([^\"' >]+)/i", $str, $match);
    ...
    */
	protected function grabPageSources() {
       ...
       /*  if(!empty($arr_src3)) $arr_src4 = $this->searchSourcesOnCSSFiles($arr_src3); */
    ...
    */
	protected function searchSources($str) {
		preg_match_all("/[img|input|embed|script]+.*[\s]*(src|background)[\040]*=[\040]*\"?([^\"' >]+)/i", $str, $arr_source);
		return $arr_source[2];
	}
    ...
    */
	protected function searchCSSClasses($str) {
		preg_match_all("/class[\040]*=[\040]*\"?([^\"' >]+)/i", $str, $arr_source);
    ...
    */
	protected function searchFrames($str) {
		preg_match_all("/frame.*[\s]*src[\040]*=[\040]*\"?([^\"' >]+)/i", $str, $arr_source);
		return $arr_source[1];
	}
    ...
    */
	protected function xsearchSourcesOnCSS($str) {
		preg_match_all("/(url\(\"?([^\")]+))/i", $str, $arr_source);
		return $arr_source[2];
	}
    ...
    */
    protected function searchSourcesOnCSSFiles($cssfiles) {
		...
		foreach ($cssfiles as $src) {
			...
			if (empty($arr_sourcelink)) {
				continue;
			}

			foreach ($arr_sourcelink as $srclink) {
    ...
    */
	protected function searchCSSLinks($str) {
        preg_match_all("/<link[^>]+href[\040]*=[\040]*[\"|\'|\\\\]*([^\'|\"|>|\040]*(.*)\.css)[\"|\'|>|\040|\\\\]*/i",$str, $arr_CSSlink);
