...
class rsseoMenuHelper
{
	public static function generateSitemap() {
		...
		// Add stylesheet
		JHtml::stylesheet('com_rsseo/site.css', array('relative' => true, 'version' => 'auto'));
	...
	protected static function render($items) {
		...
		foreach ($items as $i => &$item) {
			...
			switch ($item->type) {
				...
				case 'url':
				default:				
					$html[] = '<a href="'.JFilterOutput::ampReplace(htmlspecialchars($item->flink)).'">'.htmlspecialchars($item->title, ENT_COMPAT, 'UTF-8', false).'</a>';
				break;
			}
