...
function checkTableName($shortTName, $type=false)
{
	...
	if ("admin" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("paginas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("configuracoes" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("clientes" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("admin_users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Painel" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("admin");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="admin";
	}
	$strPerm = GetUserPermissions("paginas");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="paginas";
	}
	$strPerm = GetUserPermissions("configuracoes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="configuracoes";
	}
	$strPerm = GetUserPermissions("clientes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="clientes";
	}
	...
	$strPerm = GetUserPermissions("Painel");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Painel";
	}
...
function ReadUserPermissions($userID = "")
{
	...
	$sql = "select ". $rConnection->addFieldWrappers( "TableName" )
		...
		." from ". $rConnection->addTableWrappers( "locutoracom_ugrights" )
...
{	
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
	if(array_key_exists("admin",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("paginas",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("configuracoes",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("clientes",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("admin_users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("Painel",$_SESSION["UserRights"]["Guest"]))
		return true;
	return false;
}
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("admin")
