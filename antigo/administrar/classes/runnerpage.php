...
class RunnerPage
{
		...
		foreach($fieldsData as $fieldName => $value)
		{
			...
			if($this->hasDuplicateValue($fieldName, $value)) 
			{
				if($this->mode != EDIT_POPUP && $this->mode != ADD_POPUP)
					$message = $fieldName." "."Campo n~]ao deve conter valor duplicado";
	...
	*/
	...
	{
		if( $page == "add" )
			return GetTableCaption($table).", "."Novo Registro";
		if( $page == "edit" )
			return GetTableCaption($table).", "."Editar registro"." [". $this->getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "view" )
			return GetTableCaption($table).", "."Exibir Registro"." [". $this->getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "export" )
			return "Exportar";
		if( $page == "import" )
