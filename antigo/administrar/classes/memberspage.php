...
{	
	...
	function fillMembers()
	{
		...
		$sql = "select ". $this->connection->addFieldWrappers( "UserName" )
			.", ". $this->connection->addFieldWrappers( "GroupID" )
			." from ". $this->connection->addTableWrappers( "locutoracom_ugmembers" ) 
			." order by ". $this->connection->addFieldWrappers( "UserName" )
			.", ". $this->connection->addFieldWrappers( "GroupID" );
	...
	function fillGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "GroupID" ) .", ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "locutoracom_uggroups" ) ." order by ". $grConnection->addFieldWrappers( "Label" );
	...
	{
		// It's expected that $this->tName is equal to 'admin_members' so the page's db connection is used #9875		
		$membersWTableName = $this->connection->addTableWrappers( "locutoracom_ugmembers" );
		$userNameWFieldName = $this->connection->addFieldWrappers( "UserName" );
		$groupIdWFieldName = $this->connection->addFieldWrappers( "GroupID" );
