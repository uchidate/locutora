...
class CrossTableReport
{
	...
	{					
		...
		return "Grupo X"
			...
			.":<b>".$this->xml_array["totals"][ $fieldName ]["label"]."</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"."Função Grupo"
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("admin" == $this->xml_array['tables'][0])
			$isDB = true;
		if("paginas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("configuracoes" == $this->xml_array['tables'][0])
			$isDB = true;
		if("clientes" == $this->xml_array['tables'][0])
			$isDB = true;
		if("locutoracom_ugrights" == $this->xml_array['tables'][0])
			$isDB = true;
		if("admin" == $this->xml_array['tables'][0])
			$isDB = true;
		if("admin" == $this->xml_array['tables'][0])
