...
class ReportField
{
	...
	function ReportField($name, $interval, $alias, $table, $connection, $cipherer)
	{
		$this->_name = $name;
...
{
	function ReportNumericField($name, $interval, $alias, $table, $connection, $cipherer)
	{
		ReportField::ReportField($name, $interval, $alias, $table, $connection, $cipherer);
	}
...
{
	function ReportCharField($name, $interval, $alias, $table, $connection, $cipherer)
	{
		ReportField::ReportField($name, $interval, $alias, $table, $connection, $cipherer);
	}
...
{
	function ReportDateField($name, $interval, $alias, $table, $connection, $cipherer)
	{
		ReportField::ReportField($name, $interval, $alias, $table, $connection, $cipherer);
	}
...
class SQLStatement
{
	...
	function SQLStatement($sql, $order, $pagesize, $connection, &$params, $searchWhereClause, $searchHavingClause, $cipherer)
	{
	...
	function getSQLLimits($sql, $from)
	{
		if($from >= 0 && $this->_pagesize >= 0)
		{
			...
			elseif( $this->_connection->dbType == nDATABASE_MSSQLServer || $this->_connection->dbType == nDATABASE_Access )
			{
				...
				$out = substr_replace($sql, 'select top '.($from + $this->_pagesize), $nsel, strlen("select"));
				$this->_skipCount = $from;
			} 
			elseif( $this->_connection->dbType == nDATABASE_Oracle )
			{
				$out = 'select * from (select original2.*, rownum as '.cached_ffn('rownumber').' from ('.
					$sql.") original2) where ".cached_ffn('rownumber').' between '.$from.' and '.($from + $this->_pagesize);
				$this->_skipCount = 0;
			}
...
class ReportLogic extends Summarable
{
    ...
    function ReportLogic($sql, $order, $connection, $pagesize, &$params, $searchWhereClause, $searchHavingClause, $pageObject = null)
    {
    	RunnerApply($this, $params);
		Summarable::Summarable($params);
        $this->_connection = $connection;
		$this->cipherer = new RunnerCipherer($this->tName);
		$this->_sql = new SQLStatement($sql, $order, $pagesize, $connection, $params, $searchWhereClause, $searchHavingClause, $this->cipherer );
		$this->_groups = new ReportGroups($this->_sql, $connection, $pagesize, $params);
		$this->_pagesize = $pagesize;
		$this->_printpagesize = $pagesize;
		$this->pSet = new ProjectSettings($this->tName, PAGE_REPORT);
...
class Report extends ReportLogic
{
    ...
    function Report($sql, $order, $connection, $pagesize, &$params, $searchWhereClause, $searchHavingClause, $pageObject = null)
    {
    	// copy properties to object
		RunnerApply($this, $params);
		ReportLogic::ReportLogic($sql, $order, $connection, $pagesize, $params, $searchWhereClause, $searchHavingClause, $pageObject);
    }
