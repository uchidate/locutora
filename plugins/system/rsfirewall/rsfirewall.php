...
class plgSystemRsfirewall extends JPlugin
{
	...
	protected function isSQLi($uri)
	{
		try
		{
			...
			if (preg_match('#[\d\W](union|union select|insert|from|where|concat|into|cast|truncate|select|delete|having)[\d\W]#is', $uri, $match) && (preg_match('/'.preg_quote(JFactory::getDbo()->getPrefix()).'/', $uri, $match) || preg_match('/#__/', $uri, $match)))
			{
				throw new Exception($match[0]);
			}
		}
			...
			if ($app->isClient('administrator') && $this->config->get('backend_password_enabled') && $this->config->get('backend_password'))
			{
				...
				if ($use_parameter)
				{
					if (!$session->get('com_rsfirewall.logged_in'))
					{
						...
						if (strlen($parameter))
						{
							$password = $input->get->get($parameter, null, 'none');
						}
						else
						{
