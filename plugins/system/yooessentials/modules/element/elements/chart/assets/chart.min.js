...
(function (UIkit) {
  ...
  UIkit__default["default"].util.$$('[data-yooessentials-chart]').forEach(function (element) {
    var ctx = UIkit__default["default"].util.$('canvas', element).getContext('2d');
    var config = JSON.parse(element.dataset['yooessentialsChart']);
    var labels = JSON.parse(element.dataset['labels']);
    var datasets = JSON.parse(element.dataset['sets']);
    var defaults = {
      fontFamily: "'Helvetica Neue', 'Helvetica', 'Arial', sans-serif",
      fontColor: '#666'
    };
    var types = [].concat(getDatasetTypes(datasets), [config.type]);
    var labelsConfig = function labelsConfig(axe, label) {
      if (label === void 0) {
        label = {};
      }
      return {
        scaleLabel: {
          display: label[axe + "Label"] && label[axe + "Label"].length > 0,
          labelString: label[axe + "Label"],
          fontFamily: label.fontFamily || defaults.fontFamily,
          fontColor: label.fontColor || defaults.fontColor
        },
        ticks: {
          fontSize: label.fontSize ? parseInt(label.fontSize) : 12,
          fontColor: label.fontColor || defaults.fontColor,
          lineHeight: label.lineHeight ? parseInt(label.lineHeight) : 1,
          fontFamily: label.fontFamily || defaults.fontFamily,
    ...
    var options = {
      ...
      options: {
        ...
        legend: _extends({}, config.legend || {}, {
          fontFamily: config.fontFamily || defaults.defaultFont
        })
      }
    };

    // include scales only when required
    if (types.join(',').match(/line|bar|horizontalBar|bubble/)) {
      options.options.scales = {
        yAxes: [_extends({}, labelsConfig('y', config.labels), {
          gridLines: {
            offsetGridLines: config.type === 'horizontalBar'
          }
        })],
        xAxes: [_extends({}, labelsConfig('x', config.labels), {
          gridLines: {
            offsetGridLines: config.type === 'bar'
          }
        })]
      };
    }
    if (config.deferred) {
      options.options.plugins.deferred = {
        yOffset: '30%',
  ...
  function getDatasetTypes(datasets) {
    var types = [];
    for (var dataset in datasets) {
      types.push(datasets[dataset].type);
    }
