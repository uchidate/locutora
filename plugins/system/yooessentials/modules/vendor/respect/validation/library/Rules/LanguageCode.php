 ...
 */
final class LanguageCode extends \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Rules\AbstractEnvelope
{
    ...
    public function __construct(string $set = self::ALPHA2)
    {
        ...
        if ($index === \false) {
            throw new \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Exceptions\ComponentException(\sprintf('"%s" is not a valid language set for ISO 639', $set));
        }
        parent::__construct(new \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Rules\In($this->getHaystack($index), \true), ['set' => $set]);
    }
    /**
     * @return string[]
     */
