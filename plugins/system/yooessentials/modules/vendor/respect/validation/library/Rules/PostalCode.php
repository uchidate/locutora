 ...
 */
final class PostalCode extends \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Rules\AbstractEnvelope
{
    ...
    {
        $countryCodeRule = new \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Rules\CountryCode();
        if (!$countryCodeRule->validate($countryCode)) {
            throw new \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Exceptions\ComponentException(\sprintf('Cannot validate postal code from "%s" country', $countryCode));
        }
        parent::__construct(new \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Rules\Regex(self::POSTAL_CODES[$countryCode] ?? self::DEFAULT_PATTERN), ['countryCode' => $countryCode]);
    }
}
