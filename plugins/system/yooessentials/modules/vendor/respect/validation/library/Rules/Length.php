 ...
 */
final class Length extends \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Rules\AbstractRule
{
    /**
     * @var int|null
     */
    ...
    public function __construct(?int $min = null, ?int $max = null, bool $inclusive = \true)
    {
        ...
        if ($max !== null && $min > $max) {
            throw new \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Exceptions\ComponentException(\sprintf('%d cannot be less than %d for validation', $min, $max));
        }
    }
    /**
     * {@inheritDoc}
