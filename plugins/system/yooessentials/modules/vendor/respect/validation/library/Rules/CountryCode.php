 ...
 */
final class CountryCode extends \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Rules\AbstractSearcher
{
    /**
    ...
    public function __construct(string $set = self::ALPHA2)
    {
        if (!isset(self::SET_INDEXES[$set])) {
            throw new \ZOOlanders\YOOessentials\Vendor\Respect\Validation\Exceptions\ComponentException(\sprintf('"%s" is not a valid set for ISO 3166-1 (Available: %s)', $set, \implode(', ', \array_keys(self::SET_INDEXES))));
        }
        $this->set = $set;
    }
    /**
