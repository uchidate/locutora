 ...
 * @final
 */
class MessageFormatter implements \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\ZOOlanders\YOOessentials\Vendor\Psr\Http\Message\RequestInterface $request, ?\ZOOlanders\YOOessentials\Vendor\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
                ...
                case 'req_body':
                    ...
                    break;
                case 'res_body':
                    if (!$response instanceof \ZOOlanders\YOOessentials\Vendor\Psr\Http\Message\ResponseInterface) {
                        $result = 'NULL';
                        break;
                    }
     ...
     */
    private function headers(\ZOOlanders\YOOessentials\Vendor\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
