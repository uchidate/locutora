...
final class TableRowRenderer implements \ZOOlanders\YOOessentials\Vendor\League\CommonMark\Block\Renderer\BlockRendererInterface
{
    ...
    {
        if (!$block instanceof \ZOOlanders\YOOessentials\Vendor\League\CommonMark\Extension\Table\TableRow) {
            throw new \InvalidArgumentException('Incompatible block type: ' . \get_class($block));
        }
        $attrs = $block->getData('attributes', []);
        $separator = $htmlRenderer->getOption('inner_separator', "\n");
        return new \ZOOlanders\YOOessentials\Vendor\League\CommonMark\HtmlElement('tr', $attrs, $separator . $htmlRenderer->renderBlocks($block->children()) . $separator);
    }
}
