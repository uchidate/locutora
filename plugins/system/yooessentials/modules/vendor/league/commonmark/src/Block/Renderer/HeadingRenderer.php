...
final class HeadingRenderer implements \ZOOlanders\YOOessentials\Vendor\League\CommonMark\Block\Renderer\BlockRendererInterface
{
    /**
     * @param Heading                  $block
    ...
    {
        if (!$block instanceof \ZOOlanders\YOOessentials\Vendor\League\CommonMark\Block\Element\Heading) {
            throw new \InvalidArgumentException('Incompatible block type: ' . \get_class($block));
        }
        $tag = 'h' . $block->getLevel();
        $attrs = $block->getData('attributes', []);
        return new \ZOOlanders\YOOessentials\Vendor\League\CommonMark\HtmlElement($tag, $attrs, $htmlRenderer->renderInlines($block->children()));
    }
}
