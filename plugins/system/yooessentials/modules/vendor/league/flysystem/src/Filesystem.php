...
class Filesystem implements \ZOOlanders\YOOessentials\Vendor\League\Flysystem\FilesystemOperator
{
    /**
    ...
    public function __construct(\ZOOlanders\YOOessentials\Vendor\League\Flysystem\FilesystemAdapter $adapter, array $config = [], \ZOOlanders\YOOessentials\Vendor\League\Flysystem\PathNormalizer $pathNormalizer = null)
    {
        $this->adapter = $adapter;
        $this->config = new \ZOOlanders\YOOessentials\Vendor\League\Flysystem\Config($config);
        $this->pathNormalizer = $pathNormalizer ?: new \ZOOlanders\YOOessentials\Vendor\League\Flysystem\WhitespacePathNormalizer();
    }
    ...
    public function listContents(string $location, bool $deep = self::LIST_SHALLOW) : \ZOOlanders\YOOessentials\Vendor\League\Flysystem\DirectoryListing
    {
        ...
        return new \ZOOlanders\YOOessentials\Vendor\League\Flysystem\DirectoryListing($this->adapter->listContents($path, $deep));
    }
    ...
    private function assertIsResource($contents) : void
    {
        if (\is_resource($contents) === \false) {
            throw new \ZOOlanders\YOOessentials\Vendor\League\Flysystem\InvalidStreamProvided("Invalid stream provided, expected stream resource, received " . \gettype($contents));
        } elseif ($type = \get_resource_type($contents) !== 'stream') {
            throw new \ZOOlanders\YOOessentials\Vendor\League\Flysystem\InvalidStreamProvided("Invalid stream provided, expected stream resource, received resource of type " . $type);
        }
    }
    /**
