...
class Statement
{
    ...
    {
        if (0 > $offset) {
            throw new \ZOOlanders\YOOessentials\Vendor\League\Csv\InvalidArgument(\sprintf('%s() expects the offset to be a positive integer or 0, %s given', __METHOD__, $offset));
        }
    ...
    {
        if (-1 > $limit) {
            throw new \ZOOlanders\YOOessentials\Vendor\League\Csv\InvalidArgument(\sprintf('%s() expects the limit to be greater or equal to -1, %s given', __METHOD__, $limit));
        }
        if ($limit === $this->limit) {
            return $this;
        }
     ...
     */
    public function process(\ZOOlanders\YOOessentials\Vendor\League\Csv\TabularDataReader $tabular_data, array $header = []) : \ZOOlanders\YOOessentials\Vendor\League\Csv\TabularDataReader
    {
        if ([] === $header) {
        ...
        return new \ZOOlanders\YOOessentials\Vendor\League\Csv\ResultSet(new \LimitIterator($iterator, $this->offset, $this->limit), $header);
    }
    /**
