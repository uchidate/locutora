...
class Client
{
    ...
    public function __construct(int $accountId, string $licenseKey, array $options = [])
    {
        ...
        $this->httpRequestFactory = isset($options['httpRequestFactory']) ? $options['httpRequestFactory'] : new \ZOOlanders\YOOessentials\Vendor\MaxMind\WebService\Http\RequestFactory();
    ...
    public function post(string $service, string $path, array $input) : ?array
    {
        ...
        if ($requestBody === \false) {
            throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\InvalidInputException('Error encoding input as JSON: ' . $this->jsonErrorDescription());
        }
    ...
    private function createRequest(string $path, array $headers = []) : \ZOOlanders\YOOessentials\Vendor\MaxMind\WebService\Http\Request
    {
    ...
    {
        if ($body === null || $body === '') {
            throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\HttpException("Received a {$statusCode} error for {$service} with no body", $statusCode, $this->urlFor($path));
        }
        if ($contentType === null || !\strstr($contentType, 'json')) {
            throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\HttpException("Received a {$statusCode} error for {$service} with " . 'the following body: ' . $body, $statusCode, $this->urlFor($path));
        }
        $message = \json_decode($body, \true);
        if ($message === null) {
            throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\HttpException("Received a {$statusCode} error for {$service} but could " . 'not decode the response as JSON: ' . $this->jsonErrorDescription() . ' Body: ' . $body, $statusCode, $this->urlFor($path));
        }
        if (!isset($message['code']) || !isset($message['error'])) {
            throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\HttpException('Error response contains JSON but it does not ' . 'specify code or error keys: ' . $body, $statusCode, $this->urlFor($path));
        }
    ...
    private function handleWebServiceError(string $message, string $code, int $statusCode, string $path) : void
    {
        switch ($code) {
            ...
            case 'IP_ADDRESS_RESERVED':
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\IpAddressNotFoundException($message, $code, $statusCode, $this->urlFor($path));
            ...
            case 'USER_ID_UNKNOWN':
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\AuthenticationException($message, $code, $statusCode, $this->urlFor($path));
            case 'OUT_OF_QUERIES':
            case 'INSUFFICIENT_FUNDS':
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\InsufficientFundsException($message, $code, $statusCode, $this->urlFor($path));
            case 'PERMISSION_REQUIRED':
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\PermissionRequiredException($message, $code, $statusCode, $this->urlFor($path));
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\InvalidRequestException($message, $code, $statusCode, $this->urlFor($path));
        }
    }
    /**
     * @param int    $statusCode the HTTP status code
    ...
    {
        throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\HttpException("Received a server error ({$statusCode}) for {$service}", $statusCode, $this->urlFor($path));
    }
    /**
     * @param int    $statusCode the HTTP status code
     * @param string $service    the service name
    ...
    {
        throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\HttpException('Received an unexpected HTTP status ' . "({$statusCode}) for {$service}", $statusCode, $this->urlFor($path));
    }
    /**
     * @param int         $statusCode the HTTP status code
    ...
    private function handleSuccess(int $statusCode, ?string $body, string $service) : ?array
    {
        ...
        if ($statusCode === 204) {
            if ($body !== null && $body !== '') {
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\WebServiceException("Received a 204 response for {$service} along with an " . "unexpected HTTP body: {$body}");
            }
            return null;
        }
        // A 200 should have a valid JSON body
        if ($body === null || $body === '') {
            throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\WebServiceException("Received a 200 response for {$service} but did not " . 'receive a HTTP body.');
        }
        $decodedContent = \json_decode($body, \true);
        if ($decodedContent === null) {
            throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Exception\WebServiceException("Received a 200 response for {$service} but could " . 'not decode the response as JSON: ' . $this->jsonErrorDescription() . ' Body: ' . $body);
        }
    ...
    private function getCaBundle() : ?string
    {
        ...
        $cert = \ZOOlanders\YOOessentials\Vendor\Composer\CaBundle\CaBundle::getSystemCaRootBundlePath();
