 ...
 */
namespace ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Dumper;

use ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor;
 ...
 */
class CliDumper extends \ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Dumper\AbstractDumper
{
     ...
     * {@inheritdoc}
     */
    public function dumpScalar(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor $cursor, string $type, $value)
    {
        $this->dumpKey($cursor);
        $style = 'const';
        $attr = $cursor->attr;
     ...
     * {@inheritdoc}
     */
    public function dumpString(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor $cursor, string $str, bool $bin, int $cut)
    {
        $this->dumpKey($cursor);
        $attr = $cursor->attr;
        if ($bin) {
     ...
     * {@inheritdoc}
     */
    public function enterHash(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor $cursor, int $type, $class, bool $hasChild)
    {
        if (null === $this->colors) {
        ...
        if (\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type) {
            $prefix = $class && 'stdClass' !== $class ? $this->style('note', $class, $attr) . (empty($attr['cut_hash']) ? ' {' : '') : '{';
        } elseif (\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
            $prefix = $this->style('note', $class . ' resource', $attr) . ($hasChild ? ' {' : ' ');
        } else {
            $prefix = $class && !(self::DUMP_LIGHT_ARRAY & $this->flags) ? $this->style('note', 'array:' . $class) . ' [' : '[';
        }
        if (($cursor->softRefCount || 0 < $cursor->softRefHandle) && empty($attr['cut_hash'])) {
            $prefix .= $this->style('ref', (\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type ? '@' : '#') . (0 < $cursor->softRefHandle ? $cursor->softRefHandle : $cursor->softRefTo), ['count' => $cursor->softRefCount]);
        } elseif ($cursor->hardRefTo && !$cursor->refIndex && $class) {
            $prefix .= $this->style('ref', '&' . $cursor->hardRefTo, ['count' => $cursor->hardRefCount]);
        } elseif (!$hasChild && \ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
     ...
     * {@inheritdoc}
     */
    public function leaveHash(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor $cursor, int $type, $class, bool $hasChild, int $cut)
    {
        if (empty($cursor->attr['cut_hash'])) {
            $this->dumpEllipsis($cursor, $hasChild, $cut);
            $this->line .= \ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type ? '}' : (\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE !== $type ? ']' : ($hasChild ? '}' : ''));
        }
        $this->endValue($cursor);
    }
    /**
     ...
     */
    protected function dumpEllipsis(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor $cursor, bool $hasChild, int $cut)
    {
        if ($cut) {
            $this->line .= ' …';
            if (0 < $cut) {
     ...
     */
    protected function dumpKey(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (null !== ($key = $cursor->hashKey)) {
            ...
            switch ($cursor->hashType) {
                default:
                case \ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_INDEXED:
                    if (self::DUMP_LIGHT_ARRAY & $this->flags) {
                        break;
                    }
                    $style = 'index';
                // no break
                case \ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_ASSOC:
                    if (\is_int($key)) {
                    ...
                    break;
                case \ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE:
                    $key = "\x00~\x00" . $key;
                // no break
                case \ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT:
    ...
    protected function endValue(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (-1 === $cursor->hashType) {
            return;
        }
        if (\ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_INDEXED === $cursor->hashType || \ZOOlanders\YOOessentials\Vendor\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_ASSOC === $cursor->hashType) {
