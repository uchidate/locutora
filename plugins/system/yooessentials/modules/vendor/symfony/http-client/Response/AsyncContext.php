...
final class AsyncContext
{
    ...
    private $content;
    private $offset;
    public function __construct(&$passthru, \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\HttpClientInterface $client, \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface &$response, array &$info, $content, int $offset)
    {
     ...
     */
    public function createChunk(string $data) : \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ChunkInterface
    {
        return new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Chunk\DataChunk($this->offset, $data);
    }
    /**
     ...
     */
    public function cancel() : \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ChunkInterface
    {
        $this->info['canceled'] = \true;
        $this->info['error'] = 'Response has been canceled.';
        $this->response->cancel();
        return new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Chunk\LastChunk();
    }
    /**
     ...
     */
    public function replaceRequest(string $method, string $url, array $options = []) : \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface
    {
        ...
        if (0 < ($info['max_duration'] ?? 0) && 0 < ($info['total_time'] ?? 0)) {
            if (0 >= ($options['max_duration'] = $info['max_duration'] - $info['total_time'])) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('Max duration was reached for "%s".', $info['url']));
            }
        }
     ...
     */
    public function replaceResponse(\ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface $response) : \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface
    {
