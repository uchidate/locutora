 ...
 */
class MockResponse implements \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\StreamableInterface
{
    ...
    public function cancel() : void
    {
        ...
        try {
            $this->body = null;
        } catch (\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException $e) {
     ...
     * @internal
     */
    public static function fromRequest(string $method, string $url, array $options, \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface $mock) : self
    {
        $response = new self([]);
    ...
    {
        if (!$response->id) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException('MockResponse instances must be issued by MockHttpClient before processing.');
        }
        $multi = self::$mainMulti ?? (self::$mainMulti = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\ClientState());
     ...
     * {@inheritdoc}
     */
    protected static function perform(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\ClientState $multi, array &$responses) : void
    {
        foreach ($responses as $response) {
            ...
            if (null === $response->body) {
                ...
                $multi->handlesActivity[$id][] = null !== $response->info['error'] ? new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException($response->info['error']) : null;
     ...
     * {@inheritdoc}
     */
    protected static function select(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\ClientState $multi, float $timeout) : int
    {
        return 42;
    }
    /**
     ...
     */
    private static function writeRequest(self $response, array $options, \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface $mock)
    {
     ...
     */
    private static function readResponse(self $response, array $options, \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface $mock, int &$offset)
    {
        $onProgress = $options['on_progress'] ?? static function () {
        };
        ...
        if (null !== $response->info['error']) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException($response->info['error']);
        }
        if (!isset($response->info['total_time'])) {
            $response->info['total_time'] = \microtime(\true) - $response->info['start_time'];
        }
            ...
            try {
                foreach ($body as $chunk) {
                    if ('' === ($chunk = (string) $chunk)) {
                        ...
                        $response->body[] = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Chunk\ErrorChunk($offset, \sprintf('Idle timeout reached for "%s".', $response->info['url']));
                    } else {
                        $response->body[] = $chunk;
                        $offset += \strlen($chunk);
        ...
        if ($dlSize && $offset !== $dlSize) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('Transfer closed with %d bytes remaining to read.', $dlSize - $offset));
        }
    }
}
