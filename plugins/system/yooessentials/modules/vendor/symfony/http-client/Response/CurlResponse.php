 ...
 * @internal
 */
final class CurlResponse implements \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\StreamableInterface
{
     ...
     * @internal
     */
    public function __construct(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\CurlClientState $multi, $ch, array $options = null, \ZOOlanders\YOOessentials\Vendor\Psr\Log\LoggerInterface $logger = null, string $method = 'GET', callable $resolveRedirect = null, int $curlVersion = null)
    {
        $this->multi = $multi;
        ...
        \curl_setopt($ch, \CURLOPT_WRITEFUNCTION, static function ($ch, string $data) use($multi, $id) : int {
            if ('H' === (\curl_getinfo($ch, \CURLINFO_PRIVATE)[0] ?? null)) {
                ...
                $multi->handlesActivity[$id][] = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('Unsupported protocol for "%s"', \curl_getinfo($ch, \CURLINFO_EFFECTIVE_URL)));
                return 0;
            }
            \curl_setopt($ch, \CURLOPT_WRITEFUNCTION, static function ($ch, string $data) use($multi, $id) : int {
                $multi->handlesActivity[$id][] = $data;
        ...
        \curl_multi_add_handle($multi->handle, $ch);
        $this->canary = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\Canary(static function () use($ch, $multi, $id) {
    ...
    private static function perform(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\ClientState $multi, array &$responses = null) : void
    {
        if (self::$performing) {
            if ($responses) {
                ...
                $multi->handlesActivity[(int) $response->handle][] = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('Userland callback cannot use the client nor the response while processing "%s".', \curl_getinfo($response->handle, \CURLINFO_EFFECTIVE_URL)));
            }
            return;
        }
        try {
            ...
            if (\CURLM_OK !== $err) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\curl_multi_strerror($err));
            }
            while ($info = \curl_multi_info_read($multi->handle)) {
                ...
                $multi->handlesActivity[$id][] = null;
                $multi->handlesActivity[$id][] = \in_array($result, [\CURLE_OK, \CURLE_TOO_MANY_REDIRECTS], \true) || '_0' === $waitFor || \curl_getinfo($ch, \CURLINFO_SIZE_DOWNLOAD) === \curl_getinfo($ch, \CURLINFO_CONTENT_LENGTH_DOWNLOAD) ? null : new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\ucfirst(\curl_error($ch) ?: \curl_strerror($result)) . \sprintf(' for "%s".', \curl_getinfo($ch, \CURLINFO_EFFECTIVE_URL)));
            }
        } finally {
     ...
     */
    private static function parseHeaderLine($ch, string $data, array &$info, array &$headers, ?array $options, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\CurlClientState $multi, int $id, ?string &$location, ?callable $resolveRedirect, ?\ZOOlanders\YOOessentials\Vendor\Psr\Log\LoggerInterface $logger) : int
    {
        $waitFor = @\curl_getinfo($ch, \CURLINFO_PRIVATE) ?: '_0';
        if ('H' !== $waitFor[0]) {
            return \strlen($data);
        ...
        if (200 > ($statusCode = \curl_getinfo($ch, \CURLINFO_RESPONSE_CODE))) {
            $multi->handlesActivity[$id][] = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Chunk\InformationalChunk($statusCode, $headers);
            $location = null;
            return \strlen($data);
        }
