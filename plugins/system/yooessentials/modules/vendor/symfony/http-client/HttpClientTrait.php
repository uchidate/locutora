...
trait HttpClientTrait
{
    ...
    private static function prepareRequest(?string $method, ?string $url, array $options, array $defaultOptions = [], bool $allowExtraOptions = \false) : array
    {
        if (null !== $method) {
            if (\strlen($method) !== \strspn($method, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Invalid HTTP method "%s", only uppercase letters are accepted.', $method));
            }
            if (!$method) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException('The HTTP method cannot be empty.');
            }
        }
        ...
        if ($buffer instanceof \Closure) {
            ...
            if (!\is_array($bufferInfo = @\stream_get_meta_data($buffer))) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "buffer" must be bool, stream resource or Closure, "%s" given.', \get_debug_type($buffer)));
            }
            if (\false === \strpbrk($bufferInfo['mode'], 'acew+')) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('The stream in option "buffer" must be writeable, mode "%s" given.', $bufferInfo['mode']));
            }
        }
        if (isset($options['json'])) {
            if (isset($options['body']) && '' !== $options['body']) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException('Define either the "json" or the "body" option, setting both is not supported.');
            }
        ...
        if (\is_array($options['auth_basic'] ?? null)) {
            ...
            if ($count <= 0 || $count > 2) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "auth_basic" must contain 1 or 2 elements, "%s" given.', $count));
            }
            $options['auth_basic'] = \implode(':', $options['auth_basic']);
        }
        if (!\is_string($options['auth_basic'] ?? '')) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "auth_basic" must be string or an array, "%s" given.', \get_debug_type($options['auth_basic'])));
        }
        if (isset($options['auth_bearer'])) {
            if (!\is_string($options['auth_bearer'])) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "auth_bearer" must be a string, "%s" given.', \get_debug_type($options['auth_bearer'])));
            }
            if (\preg_match('{[^\\x21-\\x7E]}', $options['auth_bearer'])) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException('Invalid character found in option "auth_bearer": ' . \json_encode($options['auth_bearer']) . '.');
            }
        }
        if (isset($options['auth_basic'], $options['auth_bearer'])) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException('Define either the "auth_basic" or the "auth_bearer" option, setting both is not supported.');
        }
        if (null !== $url) {
    ...
    private static function mergeDefaultOptions(array $options, array $defaultOptions, bool $allowExtraOptions = \false) : array
    {
        ...
        foreach ($options as $name => $v) {
            ...
            if ('auth_ntlm' === $name) {
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "auth_ntlm" is not supported by "%s", ' . $msg, __CLASS__, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\CurlHttpClient::class));
            }
            $alternatives = [];
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Unsupported option "%s" passed to "%s", did you mean "%s"?', $name, __CLASS__, \implode('", "', $alternatives ?: \array_keys($defaultOptions))));
        }
        return $options;
    }
    /**
    ...
    private static function normalizeHeaders(array $headers) : array
    {
        ...
        foreach ($headers as $name => $values) {
            ...
            if (\is_int($name)) {
                if (!\is_string($values)) {
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Invalid value for header "%s": expected string, "%s" given.', $name, \get_debug_type($values)));
                }
            ...
            foreach ($values as $value) {
                ...
                if (\strlen($value) !== \strcspn($value, "\r\n\x00")) {
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Invalid header: CR/LF/NUL found in "%s".', $value));
                }
            }
        }
    ...
    private static function normalizeBody($body)
    {
        ...
        if (!\is_array(@\stream_get_meta_data($body))) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "body" must be string, stream resource, iterable or callable, "%s" given.', \get_debug_type($body)));
        }
        return $body;
    }
    private static function dechunk(string $body) : string
    {
        ...
        if (\fwrite($h, '-') && '' !== \stream_get_contents($h, -1, 0)) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException('Request body has broken chunked encoding.');
        }
        return $body;
    }
    /**
        ...
        if (\is_string($fingerprint)) {
            switch (\strlen($fingerprint = \str_replace(':', '', $fingerprint))) {
                ...
                case 64:
                    ...
                    break;
                default:
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Cannot auto-detect fingerprint algorithm for "%s".', $fingerprint));
            }
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "peer_fingerprint" must be string or array, "%s" given.', \get_debug_type($fingerprint)));
        }
        return $fingerprint;
    }
    /**
    ...
    private static function jsonEncode($value, int $flags = null, int $maxDepth = 512) : string
    {
        ...
        try {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException('Invalid value for "json" option: ' . $e->getMessage());
        }
        if (\PHP_VERSION_ID < 70300 && \JSON_ERROR_NONE !== \json_last_error() && (\false === $value || !($flags & \JSON_PARTIAL_OUTPUT_ON_ERROR))) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException('Invalid value for "json" option: ' . \json_last_error_msg());
        }
        return $value;
    }
    /**
    ...
    private static function resolveUrl(array $url, ?array $base, array $queryDefaults = []) : array
    {
        if (null !== $base && '' === ($base['scheme'] ?? '') . ($base['authority'] ?? '')) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Invalid "base_uri" option: host or scheme is missing in "%s".', \implode('', $base)));
        }
        if (null === $url['scheme'] && (null === $base || null === $base['scheme'])) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Invalid URL: scheme is missing in "%s". Did you forget to add "http(s)://"?', \implode('', $base ?? $url)));
        }
        if (null === $base && '' === $url['scheme'] . $url['authority']) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Invalid URL: no "base_uri" option was provided and host or scheme is missing in "%s".', \implode('', $url)));
        }
        if (null !== $url['scheme']) {
    ...
    private static function parseUrl(string $url, array $query = [], array $allowedSchemes = ['http' => 80, 'https' => 443]) : array
    {
        if (\false === ($parts = \parse_url($url))) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Malformed URL "%s".', $url));
        }
        if ($query) {
        ...
        if (null !== ($scheme = $parts['scheme'] ?? null)) {
            if (!isset($allowedSchemes[$scheme = \strtolower($scheme)])) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Unsupported scheme in "%s".', $url));
            }
        ...
        if (null !== ($host = $parts['host'] ?? null)) {
            if (!\defined('INTL_IDNA_VARIANT_UTS46') && \preg_match('/[\\x80-\\xFF]/', $host)) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Unsupported IDN "%s", try enabling the "intl" PHP extension or running "composer require symfony/polyfill-intl-idn".', $host));
            }
    ...
    private static function getProxy(?string $proxy, array $url, ?string $noProxy) : ?array
    {
        ...
        if (!isset($proxy['host'])) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException('Invalid HTTP proxy: host is missing.');
        }
        if ('http' === $proxy['scheme']) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('Unsupported proxy scheme "%s": "http" or "https" expected.', $proxy['scheme']));
        }
