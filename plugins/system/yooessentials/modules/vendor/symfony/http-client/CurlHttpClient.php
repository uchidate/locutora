 ...
 */
final class CurlHttpClient implements \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\HttpClientInterface, \ZOOlanders\YOOessentials\Vendor\Psr\Log\LoggerAwareInterface, \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\Service\ResetInterface
{
    use HttpClientTrait;
    ...
    public function __construct(array $defaultOptions = [], int $maxHostConnections = 6, int $maxPendingPushes = 50)
    {
        ...
        $this->multi = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\CurlClientState($maxHostConnections, $maxPendingPushes);
    }
    ...
    public function request(string $method, string $url, array $options = []) : \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface
    {
        ...
        } elseif (\defined('CURL_VERSION_HTTP2') && \CURL_VERSION_HTTP2 & \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\CurlClientState::$curlVersion['features'] && ('https:' === $scheme || 2.0 === (float) $options['http_version'])) {
        ...
        if (isset($options['auth_ntlm'])) {
            ...
            if (\is_array($options['auth_ntlm'])) {
                ...
                if ($count <= 0 || $count > 2) {
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "auth_ntlm" must contain 1 or 2 elements, %d given.', $count));
                }
                $options['auth_ntlm'] = \implode(':', $options['auth_ntlm']);
            }
            if (!\is_string($options['auth_ntlm'])) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Option "auth_ntlm" must be a string or an array, "%s" given.', \get_debug_type($options['auth_ntlm'])));
            }
        ...
        if ($options['resolve'] || $this->multi->dnsCache->evictions) {
            ...
            if ($resolve && 0x72a00 > \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\CurlClientState::$curlVersion['version_number']) {
        ...
        foreach ($curlopts as $opt => $value) {
            if (null !== $value && !\curl_setopt($ch, $opt, $value) && \CURLOPT_CERTINFO !== $opt && (!\defined('CURLOPT_HEADEROPT') || \CURLOPT_HEADEROPT !== $opt)) {
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('Curl option "%s" is not supported.', $constantName ?? $opt));
            }
        }
        return $pushedResponse ?? new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\CurlResponse($this->multi, $ch, $options, $this->logger, $method, self::createRedirectResolver($options, $host), \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\CurlClientState::$curlVersion['version_number']);
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        if ($responses instanceof \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\CurlResponse) {
            $responses = [$responses];
        ...
        return new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\ResponseStream(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\CurlResponse::stream($responses, $timeout));
    }
     ...
     */
    private static function acceptPushForRequest(string $method, array $options, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\PushedResponse $pushedResponse) : bool
    {
    ...
    private static function readRequestBody(int $length, \Closure $body, string &$buffer, bool &$eof) : string
    {
        if (!$eof && \strlen($buffer) < $length) {
            if (!\is_string($data = $body($length))) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('The return value of the "body" option callback must be a string, "%s" returned.', \get_debug_type($data)));
            }
            $buffer .= $data;
            $eof = '' === $data;
        }
    ...
    private function validateExtraCurlOptions(array $options) : void
    {
        ...
        foreach ($options as $opt => $optValue) {
            if (isset($curloptsToConfig[$opt])) {
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Cannot set "%s" with "extra.curl", use option "%s" instead.', $constName, $curloptsToConfig[$opt]));
            }
            if (\in_array($opt, $methodOpts)) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException('The HTTP method cannot be overridden using "extra.curl".');
            }
            if (\in_array($opt, $curloptsToCheck)) {
                $constName = $this->findConstantName($opt) ?? $opt;
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\InvalidArgumentException(\sprintf('Cannot set "%s" with "extra.curl".', $constName));
            }
        }
    }
}
