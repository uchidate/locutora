 ...
 */
final class NativeHttpClient implements \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\HttpClientInterface, \ZOOlanders\YOOessentials\Vendor\Psr\Log\LoggerAwareInterface, \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\Service\ResetInterface
{
    use HttpClientTrait;
    ...
    public function __construct(array $defaultOptions = [], int $maxHostConnections = 6)
    {
        ...
        $this->multi = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\NativeClientState();
    ...
    public function request(string $method, string $url, array $options = []) : \ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\ResponseInterface
    {
        ...
        if ($options['bindto']) {
            if (\file_exists($options['bindto'])) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(__CLASS__ . ' cannot bind to local Unix sockets, use e.g. CurlHttpClient instead.');
            }
        ...
        if ($options['peer_fingerprint']) {
            if (isset($options['peer_fingerprint']['pin-sha256']) && 1 === \count($options['peer_fingerprint'])) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(__CLASS__ . ' cannot verify "pin-sha256" fingerprints, please provide a "sha256" one.');
            }
        ...
        if ($onProgress = $options['on_progress']) {
            ...
            $onProgress = static function (...$progress) use($onProgress, &$lastProgress, &$info, $maxDuration) {
                if ($info['total_time'] >= $maxDuration) {
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('Max duration was reached for "%s".', \implode('', $info['url'])));
                }
                $progressInfo = $info;
        ...
        return new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\NativeResponse($this->multi, $context, \implode('', $url), $options, $info, $resolver, $onProgress, $this->logger);
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        if ($responses instanceof \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\NativeResponse) {
            $responses = [$responses];
        ...
        return new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\ResponseStream(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Response\NativeResponse::stream($responses, $timeout));
    }
     ...
     */
    private static function dnsResolve($host, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\NativeClientState $multi, array &$info, ?\Closure $onProgress) : string
    {
        if (null === ($ip = $multi->dnsCache[$host] ?? null)) {
            $info['debug'] .= "* Hostname was NOT found in DNS cache\n";
            $now = \microtime(\true);
            if (!($ip = \gethostbynamel($host))) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Exception\TransportException(\sprintf('Could not resolve host "%s".', $host));
            }
    ...
    private static function createRedirectResolver(array $options, string $host, ?array $proxy, array &$info, ?\Closure $onProgress) : \Closure
    {
        ...
        return static function (\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\Internal\NativeClientState $multi, ?string $location, $context) use(&$redirectHeaders, $proxy, &$info, $maxRedirects, $onProgress) : ?string {
