...
class PdoAdapter extends \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Adapter\AbstractAdapter implements \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\PruneableInterface
{
    ...
    {
        if ($connOrDsn instanceof \ZOOlanders\YOOessentials\Vendor\Doctrine\DBAL\Connection || \is_string($connOrDsn) && \str_contains($connOrDsn, '://')) {
            trigger_deprecation('symfony/cache', '5.4', 'Usage of a DBAL Connection with "%s" is deprecated and will be removed in symfony 6.0. Use "%s" instead.', __CLASS__, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Adapter\DoctrineDbalAdapter::class);
            $this->dbalAdapter = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Adapter\DoctrineDbalAdapter($connOrDsn, $namespace, $defaultLifetime, $options, $marshaller);
            return;
        }
        ...
        if ($connOrDsn instanceof \PDO) {
            if (\PDO::ERRMODE_EXCEPTION !== $connOrDsn->getAttribute(\PDO::ATTR_ERRMODE)) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" requires PDO error mode attribute be set to throw Exceptions (i.e. $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION)).', __CLASS__));
            }
            $this->conn = $connOrDsn;
        } elseif (\is_string($connOrDsn)) {
            $this->dsn = $connOrDsn;
        } else {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" requires PDO or Doctrine\\DBAL\\Connection instance or DSN string as first argument, "%s" given.', __CLASS__, \get_debug_type($connOrDsn)));
        }
