 ...
 */
class MemcachedAdapter extends \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Adapter\AbstractAdapter
{
    /**
     ...
     */
    public function __construct(\Memcached $client, string $namespace = '', int $defaultLifetime = 0, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if (!static::isSupported()) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\CacheException('Memcached ' . (\PHP_VERSION_ID >= 80100 ? '> 3.1.5' : '>= 2.2.0') . ' is required.');
        }
        if ('Memcached' === \get_class($client)) {
            $opt = $client->getOption(\Memcached::OPT_SERIALIZER);
            if (\Memcached::SERIALIZER_PHP !== $opt && \Memcached::SERIALIZER_IGBINARY !== $opt) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\CacheException('MemcachedAdapter: "serializer" option must be "php" or "igbinary".');
            }
    ...
    public static function createConnection($servers, array $options = [])
    {
        if (\is_string($servers)) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('MemcachedAdapter::createClient() expects array or string as first argument, "%s" given.', \get_debug_type($servers)));
        }
        if (!static::isSupported()) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\CacheException('Memcached ' . (\PHP_VERSION_ID >= 80100 ? '> 3.1.5' : '>= 2.2.0') . ' is required.');
        }
        ...
        try {
            ...
            foreach ($servers as $i => $dsn) {
                ...
                if (!\str_starts_with($dsn, 'memcached:')) {
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Memcached DSN: "%s" does not start with "memcached:".', $dsn));
                }
                ...
                if (\false === ($params = \parse_url($params))) {
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Memcached DSN: "%s".', $dsn));
                }
                $query = $hosts = [];
    ...
    private function checkResultCode($result)
    {
        ...
        throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\CacheException('MemcachedAdapter client error: ' . \strtolower($this->client->getResultMessage()));
    }
    private function getClient() : \Memcached
    {
        ...
        if ('' !== ($prefix = (string) $this->lazyClient->getOption(\Memcached::OPT_PREFIX_KEY))) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\CacheException(\sprintf('MemcachedAdapter: "prefix_key" option must be empty when using proxified connections, "%s" given.', $prefix));
        }
