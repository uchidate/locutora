 ...
 */
class CouchbaseBucketAdapter extends \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Adapter\AbstractAdapter
{
    ...
    private $bucket;
    private $marshaller;
    public function __construct(\ZOOlanders\YOOessentials\Vendor\CouchbaseBucket $bucket, string $namespace = '', int $defaultLifetime = 0, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if (!static::isSupported()) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\CacheException('Couchbase >= 2.6.0 < 3.0.0 is required.');
        }
        ...
        try {
            ...
            foreach ($servers as $dsn) {
                if (0 !== \strpos($dsn, 'couchbase:')) {
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Couchbase DSN: "%s" does not start with "couchbase:".', $dsn));
                }
