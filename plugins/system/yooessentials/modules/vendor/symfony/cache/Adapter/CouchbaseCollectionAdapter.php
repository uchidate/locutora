...
use ZOOlanders\YOOessentials\Vendor\Couchbase\ClusterOptions;
use ZOOlanders\YOOessentials\Vendor\Couchbase\Collection;
use ZOOlanders\YOOessentials\Vendor\Couchbase\DocumentNotFoundException;
use ZOOlanders\YOOessentials\Vendor\Couchbase\UpsertOptions;
 ...
 */
class CouchbaseCollectionAdapter extends \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Adapter\AbstractAdapter
{
    private const MAX_KEY_LENGTH = 250;
    /** @var Collection */
    private $connection;
    private $marshaller;
    public function __construct(\ZOOlanders\YOOessentials\Vendor\Couchbase\Collection $connection, string $namespace = '', int $defaultLifetime = 0, \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if (!static::isSupported()) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\CacheException('Couchbase >= 3.0.0 < 4.0.0 is required.');
        }
        ...
        try {
            ...
            foreach ($dsn as $server) {
                if (0 !== \strpos($server, 'couchbase:')) {
                    throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Couchbase DSN: "%s" does not start with "couchbase:".', $server));
                }
            ...
            $clusterOptions = new \ZOOlanders\YOOessentials\Vendor\Couchbase\ClusterOptions();
    ...
    protected function doFetch(array $ids) : array
    {
        ...
        foreach ($ids as $id) {
            ...
            } catch (\ZOOlanders\YOOessentials\Vendor\Couchbase\DocumentNotFoundException $exception) {
                continue;
            }
    ...
    protected function doSave(array $values, $lifetime)
    {
        ...
        $upsertOptions = new \ZOOlanders\YOOessentials\Vendor\Couchbase\UpsertOptions();
