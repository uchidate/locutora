 ...
 */
class DataPart extends \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\TextPart
{
    /** @internal */
    protected $_parent;
    ...
    public static function fromPath(string $path, string $name = null, string $contentType = null) : self
    {
        if (null === $contentType) {
            ...
            if (null === self::$mimeTypes) {
                self::$mimeTypes = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\MimeTypes();
            }
            $contentType = self::$mimeTypes->getMimeTypes($ext)[0] ?? 'application/octet-stream';
        }
        if (\is_file($path) && !\is_readable($path) || \is_dir($path)) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('Path "%s" is not readable.', $path));
        }
        if (\false === ($handle = @\fopen($path, 'r', \false))) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('Unable to open path "%s".', $path));
        }
        if (!\is_file($path)) {
    ...
    public function __sleep()
    {
        ...
        foreach (['body', 'charset', 'subtype', 'disposition', 'name', 'encoding'] as $name) {
            $r = new \ReflectionProperty(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\TextPart::class, $name);
            $r->setAccessible(\true);
