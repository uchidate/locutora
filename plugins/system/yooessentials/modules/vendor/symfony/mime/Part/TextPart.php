 ...
 */
class TextPart extends \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\AbstractPart
{
    /** @internal */
    protected $_headers;
    ...
    public function __construct($body, ?string $charset = 'utf-8', string $subtype = 'plain', string $encoding = null)
    {
        ...
        if (null === $encoding) {
            ...
            if ('quoted-printable' !== $encoding && 'base64' !== $encoding && '8bit' !== $encoding) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('The encoding must be one of "quoted-printable", "base64", or "8bit" ("%s" given).', $encoding));
            }
    ...
    private function getEncoder() : \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Encoder\ContentEncoderInterface
    {
        if ('8bit' === $this->encoding) {
            return self::$encoders[$this->encoding] ?? (self::$encoders[$this->encoding] = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Encoder\EightBitContentEncoder());
        }
        if ('quoted-printable' === $this->encoding) {
            return self::$encoders[$this->encoding] ?? (self::$encoders[$this->encoding] = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Encoder\QpContentEncoder());
        }
        return self::$encoders[$this->encoding] ?? (self::$encoders[$this->encoding] = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Encoder\Base64ContentEncoder());
    }
    ...
    {
        $r = new \ReflectionProperty(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\AbstractPart::class, 'headers');
        $r->setAccessible(\true);
