 ...
 */
class Email extends \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Message
{
    ...
    {
        return $this->setHeaderBody('Path', 'Return-Path', \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Address::create($address));
    }
    public function getReturnPath() : ?\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Address
    {
    ...
    {
        return $this->setHeaderBody('Mailbox', 'Sender', \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Address::create($address));
    }
    public function getSender() : ?\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Address
    {
     ...
     * @return $this
     */
    public function attachPart(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\DataPart $part)
    {
        $this->cachedBody = null;
    ...
    public function ensureValidity()
    {
        if (null === $this->text && null === $this->html && !$this->attachments) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Exception\LogicException('A message must have a text or an HTML part or attachments.');
        }
     ...
     */
    private function generateBody() : \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\AbstractPart
    {
        ...
        $part = null === $this->text ? null : new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\TextPart($this->text, $this->textCharset);
        if (null !== $htmlPart) {
            if (null !== $part) {
                $part = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\Multipart\AlternativePart($part, $htmlPart);
            } else {
                $part = $htmlPart;
            }
        }
        if ($relatedParts) {
            $part = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\Multipart\RelatedPart($part, ...$relatedParts);
        }
        if ($otherParts) {
            if ($part) {
                $part = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\Multipart\MixedPart($part, ...$otherParts);
            } else {
                $part = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\Multipart\MixedPart(...$otherParts);
            }
        }
    ...
    private function prepareParts() : ?array
    {
        ...
        $html = $this->html;
        if (null !== $html) {
            $htmlPart = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\TextPart($html, $this->htmlCharset, 'html');
        ...
        $nameRef = new \ReflectionProperty(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\TextPart::class, 'name');
    ...
    private function createDataPart(array $attachment) : \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\DataPart
    {
        if (isset($attachment['part'])) {
            return $attachment['part'];
        }
        if (isset($attachment['body'])) {
            $part = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\DataPart($attachment['body'], $attachment['name'] ?? null, $attachment['content-type'] ?? null);
        } else {
            $part = \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\DataPart::fromPath($attachment['path'] ?? '', $attachment['name'] ?? null, $attachment['content-type'] ?? null);
        }
    ...
    private function addListAddressHeaderBody(string $name, array $addresses)
    {
        ...
        $header->addAddresses(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Address::createArray($addresses));
        return $this;
    }
    /**
     * @return $this
    ...
    {
        $addresses = \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Address::createArray($addresses);
    ...
    public function __serialize() : array
    {
        if (\is_resource($this->text)) {
            $this->text = (new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\TextPart($this->text))->getBody();
        }
        if (\is_resource($this->html)) {
            $this->html = (new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\TextPart($this->html))->getBody();
        }
        foreach ($this->attachments as $i => $attachment) {
            if (isset($attachment['body']) && \is_resource($attachment['body'])) {
                $this->attachments[$i]['body'] = (new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\TextPart($attachment['body']))->getBody();
            }
        }
