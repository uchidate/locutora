...
final class DkimSigner
{
    ...
    public function __construct(string $pk, string $domainName, string $selector, array $defaultOptions = [], string $passphrase = '')
    {
        ...
        if (!($this->key = \openssl_pkey_get_private($pk, $passphrase))) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Exception\InvalidArgumentException('Unable to load DKIM private key: ' . \openssl_error_string());
        }
    ...
    public function sign(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Message $message, array $options = []) : \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Message
    {
        $options += $this->defaultOptions;
        if (!\in_array($options['algorithm'], [self::ALGO_SHA256, self::ALGO_ED25519], \true)) {
            throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('Invalid DKIM signing algorithm "%s".', $options['algorithm']));
        }
        ...
        $value = \trim($value);
        $header = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Header\UnstructuredHeader('DKIM-Signature', $value);
        ...
        if (self::ALGO_SHA256 === $options['algorithm']) {
            if (!\openssl_sign($headerCanonData, $signature, $this->key, \OPENSSL_ALGO_SHA256)) {
                throw new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Exception\RuntimeException('Unable to sign DKIM hash: ' . \openssl_error_string());
            }
        } else {
        ...
        $headers->add($header);
        return new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Message($headers, $message->getBody());
    }
    ...
    private function hashBody(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\Mime\Part\AbstractPart $body, string $bodyCanon, int $maxLength) : array
    {
