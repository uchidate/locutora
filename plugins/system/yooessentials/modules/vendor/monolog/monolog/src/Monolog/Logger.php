...
use DateTimeZone;
use ZOOlanders\YOOessentials\Vendor\Monolog\Handler\HandlerInterface;
use ZOOlanders\YOOessentials\Vendor\Psr\Log\LoggerInterface;
use ZOOlanders\YOOessentials\Vendor\Psr\Log\InvalidArgumentException;
 ...
 */
class Logger implements \ZOOlanders\YOOessentials\Vendor\Psr\Log\LoggerInterface, \ZOOlanders\YOOessentials\Vendor\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\ZOOlanders\YOOessentials\Vendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], \ZOOlanders\YOOessentials\Vendor\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \ZOOlanders\YOOessentials\Vendor\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \ZOOlanders\YOOessentials\Vendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \ZOOlanders\YOOessentials\Vendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName(int $level) : string
    {
        if (!isset(static::$levels[$level])) {
            throw new \ZOOlanders\YOOessentials\Vendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
    ...
    public static function toMonologLevel($level) : int
    {
        if (\is_string($level)) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        if (!\is_int($level)) {
            throw new \ZOOlanders\YOOessentials\Vendor\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        return $level;
    }
    /**
