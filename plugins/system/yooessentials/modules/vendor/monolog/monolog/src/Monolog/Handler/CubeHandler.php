 ...
 */
class CubeHandler extends \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $url, $level = \ZOOlanders\YOOessentials\Vendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
    ...
    protected function connectUdp() : void
    {
        if (!\extension_loaded('sockets')) {
            throw new \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\MissingExtensionException('The sockets extension is required to use udp URLs with the CubeHandler');
        }
    ...
    protected function connectHttp() : void
    {
        if (!\extension_loaded('curl')) {
            throw new \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\MissingExtensionException('The curl extension is required to use http URLs with the CubeHandler');
        }
    ...
    protected function write(array $record) : void
    {
        ...
        if ($this->scheme === 'http') {
            $this->writeHttp(\ZOOlanders\YOOessentials\Vendor\Monolog\Utils::jsonEncode($data));
        } else {
            $this->writeUdp(\ZOOlanders\YOOessentials\Vendor\Monolog\Utils::jsonEncode($data));
        }
    }
    ...
    private function writeHttp(string $data) : void
    {
        ...
        \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\Curl\Util::execute($this->httpConnection, 5, \false);
    }
}
