...
use ZOOlanders\YOOessentials\Vendor\Monolog\Formatter\LogglyFormatter;
 ...
 */
class LogglyHandler extends \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \ZOOlanders\YOOessentials\Vendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \ZOOlanders\YOOessentials\Vendor\Monolog\Formatter\FormatterInterface
    {
        return new \ZOOlanders\YOOessentials\Vendor\Monolog\Formatter\LogglyFormatter();
    }
}
