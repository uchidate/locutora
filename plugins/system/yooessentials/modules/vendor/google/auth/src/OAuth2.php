...
use ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Query;
 ...
 */
class OAuth2 implements \ZOOlanders\YOOessentials\Vendor\Google\Auth\FetchAuthTokenInterface
{
    ...
    public function toJwt(array $config = [])
    {
        ...
        return \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\JWT::encode($assertion, $this->getSigningKey(), $this->getSigningAlgorithm(), $this->getSigningKeyId());
    }
    /**
    ...
    public function generateCredentialsRequest()
    {
        ...
        return new \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Request('POST', $uri, $headers, \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Query::build($params));
    }
    /**
     ...
     * @throws \Exception
     */
    public function parseTokenResponse(\ZOOlanders\YOOessentials\Vendor\Psr\Http\Message\ResponseInterface $resp)
    {
    ...
    public function buildFullAuthorizationUri(array $config = [])
    {
        ...
        $existingParams = \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Query::parse($result->getQuery());
        $result = $result->withQuery(\ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Query::build(\array_merge($existingParams, $params)));
    ...
    private function coerceUri($uri)
    {
        ...
        return \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Utils::uriFor($uri);
    }
    /**
     * @param string $idToken
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        ...
        foreach ($keys as $key) {
            try {
                return \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\JWT::decode($idToken, $key);
            } catch (\Exception $e) {
                // try next alg
            }
        }
    ...
    private function getFirebaseJwtKeys($publicKey, $allowedAlgs)
    {
        ...
        if ($publicKey instanceof \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key) {
            return [$publicKey];
        }
        ...
        if (empty($allowedAlgs)) {
            ...
            foreach ((array) $publicKey as $kid => $pubKey) {
                if (!$pubKey instanceof \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key) {
                    throw new \InvalidArgumentException(\sprintf('When allowed algorithms is empty, the public key must' . 'be an instance of %s or an array of %s objects', \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key::class, \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key::class));
                }
                $keys[$kid] = $pubKey;
            }
            return $keys;
        ...
        if (\is_array($publicKey)) {
            ...
            foreach ($publicKey as $kid => $pubKey) {
                if ($pubKey instanceof \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key) {
                    ...
                    $keys[$kid] = new \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key($pubKey, $allowedAlg);
                }
            }
            return $keys;
        }
        return [new \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key($publicKey, $allowedAlg)];
    }
    /**
