...
trait ServiceAccountSignerTrait
{
    ...
    public function signBlob($stringToSign, $forceOpenssl = \false)
    {
        ...
        $signedString = '';
        if (\class_exists('ZOOlanders\\YOOessentials\\Vendor\\phpseclib\\Crypt\\RSA') && !$forceOpenssl) {
            $rsa = new \ZOOlanders\YOOessentials\Vendor\phpseclib\Crypt\RSA();
            $rsa->loadKey($privateKey);
            $rsa->setSignatureMode(\ZOOlanders\YOOessentials\Vendor\phpseclib\Crypt\RSA::SIGNATURE_PKCS1);
            $rsa->setHash('sha256');
