...
use DateTime;
use Exception;
use ZOOlanders\YOOessentials\Vendor\Firebase\JWT\ExpiredException;
use ZOOlanders\YOOessentials\Vendor\Firebase\JWT\JWT;
use ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key;
use ZOOlanders\YOOessentials\Vendor\Firebase\JWT\SignatureInvalidException;
...
use ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Request;
use ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Utils;
...
use ZOOlanders\YOOessentials\Vendor\SimpleJWT\InvalidTokenException;
use ZOOlanders\YOOessentials\Vendor\SimpleJWT\JWT as SimpleJWT;
use ZOOlanders\YOOessentials\Vendor\SimpleJWT\Keys\KeyFactory;
use ZOOlanders\YOOessentials\Vendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
     ...
     */
    public function __construct(callable $httpHandler = null, \ZOOlanders\YOOessentials\Vendor\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->httpHandler = $httpHandler ?: \ZOOlanders\YOOessentials\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ZOOlanders\YOOessentials\Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        $this->cache = $cache ?: new \ZOOlanders\YOOessentials\Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    public function verify($token, array $options = [])
    {
        ...
        } catch (\ZOOlanders\YOOessentials\Vendor\Firebase\JWT\ExpiredException $e) {
            // firebase/php-jwt 5+
        } catch (\ZOOlanders\YOOessentials\Vendor\Firebase\JWT\SignatureInvalidException $e) {
            // firebase/php-jwt 5+
        } catch (\ZOOlanders\YOOessentials\Vendor\SimpleJWT\InvalidTokenException $e) {
            // simplejwt
    ...
    {
        $this->checkSimpleJwt();
        $jwkset = new \ZOOlanders\YOOessentials\Vendor\SimpleJWT\Keys\KeySet();
        foreach ($certs as $cert) {
            $jwkset->add(\ZOOlanders\YOOessentials\Vendor\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $rsa = new \ZOOlanders\YOOessentials\Vendor\phpseclib\Crypt\RSA();
            $rsa->loadKey(['n' => new \ZOOlanders\YOOessentials\Vendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['n']]), 256), 'e' => new \ZOOlanders\YOOessentials\Vendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['e']]), 256)]);
            // create an array of key IDs to certs for the JWT library
            $keys[$cert['kid']] = new \ZOOlanders\YOOessentials\Vendor\Firebase\JWT\Key($rsa->getPublicKey(), 'RS256');
        }
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $body = \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Utils::streamFor(\http_build_query(['token' => $token]));
        $request = new \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    private function checkAndInitializePhpsec()
    {
        ...
        if (!\class_exists('ZOOlanders\\YOOessentials\\Vendor\\phpseclib\\Crypt\\RSA')) {
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists(\ZOOlanders\YOOessentials\Vendor\SimpleJWT\JWT::class)) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            ...
            if (!\defined('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_MODE')) {
                \define('ZOOlanders\\YOOessentials\\Vendor\\CRYPT_RSA_MODE', \ZOOlanders\YOOessentials\Vendor\phpseclib\Crypt\RSA::MODE_OPENSSL);
            }
        }
    }
    /**
    ...
    {
        return \call_user_func_array([\ZOOlanders\YOOessentials\Vendor\Firebase\JWT\JWT::class, $method], $args);
        // @phpstan-ignore-line
    }
    /**
     * Provide a hook to mock calls to the JWT static methods.
    ...
    {
        return \call_user_func_array([\ZOOlanders\YOOessentials\Vendor\SimpleJWT\JWT::class, 'decode'], $args);
    }
    /**
