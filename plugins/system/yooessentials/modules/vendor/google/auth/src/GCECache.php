...
class GCECache
{
     ...
     */
    public function __construct(array $cacheConfig = null, \ZOOlanders\YOOessentials\Vendor\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->cache = $cache;
    ...
    public function onGce(callable $httpHandler = null)
    {
        if (\is_null($this->cache)) {
            return \ZOOlanders\YOOessentials\Vendor\Google\Auth\Credentials\GCECredentials::onGce($httpHandler);
        }
        $cacheKey = self::GCE_CACHE_KEY;
        $onGce = $this->getCachedValue($cacheKey);
        if (\is_null($onGce)) {
            $onGce = \ZOOlanders\YOOessentials\Vendor\Google\Auth\Credentials\GCECredentials::onGce($httpHandler);
