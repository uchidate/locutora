...
class Batch
{
    ...
    private $rootUrl;
    private $batchPath;
    public function __construct(\ZOOlanders\YOOessentials\Vendor\Google\Client $client, $boundary = \false, $rootUrl = null, $batchPath = null)
    {
        $this->client = $client;
    ...
    public function add(\ZOOlanders\YOOessentials\Vendor\Psr\Http\Message\RequestInterface $request, $key = \false)
    {
        if (\false == $key) {
            $key = \mt_rand();
        }
...
EOF;
        ...
        $request = new \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Request('POST', $url, $headers, $body);
        $response = $this->client->execute($request);
        return $this->parseResponse($response, $classes);
    }
    public function parseResponse(\ZOOlanders\YOOessentials\Vendor\Psr\Http\Message\ResponseInterface $response, $classes = [])
    {
        ...
        if (!empty($body)) {
            ...
            foreach ($parts as $i => $part) {
                ...
                if (!empty($part)) {
                    ...
                    $response = new \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Response((int) $status, $partHeaders, \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Utils::streamFor($partBody));
                    // Need content id.
                    $key = $headers['content-id'];
                    try {
                        $response = \ZOOlanders\YOOessentials\Vendor\Google\Http\REST::decodeHttpResponse($response, $requests[$i - 1]);
                    } catch (\ZOOlanders\YOOessentials\Vendor\Google\Service\Exception $e) {
