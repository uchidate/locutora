...
use ZOOlanders\YOOessentials\Vendor\Google\Http\MediaFileUpload;
use ZOOlanders\YOOessentials\Vendor\Google\Model;
use ZOOlanders\YOOessentials\Vendor\Google\Utils\UriTemplate;
...
class Resource
{
    ...
    public function call($name, $arguments, $expectedClass = null)
    {
        if (!isset($this->methods[$name])) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\Google\Exception("Unknown function: " . "{$this->serviceName}->{$this->resourceName}->{$name}()");
        }
        ...
        if (isset($parameters['postBody'])) {
            if ($parameters['postBody'] instanceof \ZOOlanders\YOOessentials\Vendor\Google\Model) {
        ...
        foreach ($parameters as $key => $val) {
            if ($key != 'postBody' && !isset($method['parameters'][$key])) {
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\Google\Exception("({$name}) unknown parameter: '{$key}'");
            }
        }
        foreach ($method['parameters'] as $paramName => $paramSpec) {
            if (isset($paramSpec['required']) && $paramSpec['required'] && !isset($parameters[$paramName])) {
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\Google\Exception("({$name}) missing required param: '{$paramName}'");
            }
        ...
        $request = new \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Psr7\Request($method['httpMethod'], $url, ['content-type' => 'application/json'], $postBody ? \json_encode($postBody) : '');
        // support uploads
        if (isset($parameters['data'])) {
            $mimeType = isset($parameters['mimeType']) ? $parameters['mimeType']['value'] : 'application/octet-stream';
            $data = $parameters['data']['value'];
            $upload = new \ZOOlanders\YOOessentials\Vendor\Google\Http\MediaFileUpload($this->client, $request, $mimeType, $data);
    ...
    public function createRequestUri($restPath, $params)
    {
        ...
        if (\count($uriTemplateVars)) {
            $uriTemplateParser = new \ZOOlanders\YOOessentials\Vendor\Google\Utils\UriTemplate();
