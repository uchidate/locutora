 ...
 */
namespace ZOOlanders\YOOessentials\Vendor\Google;

use BadMethodCallException;
use DomainException;
use ZOOlanders\YOOessentials\Vendor\Google\AccessToken\Revoke;
use ZOOlanders\YOOessentials\Vendor\Google\AccessToken\Verify;
use ZOOlanders\YOOessentials\Vendor\Google\Auth\ApplicationDefaultCredentials;
use ZOOlanders\YOOessentials\Vendor\Google\Auth\Cache\MemoryCacheItemPool;
use ZOOlanders\YOOessentials\Vendor\Google\Auth\Credentials\ServiceAccountCredentials;
use ZOOlanders\YOOessentials\Vendor\Google\Auth\Credentials\UserRefreshCredentials;
...
use ZOOlanders\YOOessentials\Vendor\Google\Auth\OAuth2;
use ZOOlanders\YOOessentials\Vendor\Google\AuthHandler\AuthHandlerFactory;
use ZOOlanders\YOOessentials\Vendor\Google\Http\REST;
use ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Client as GuzzleClient;
use ZOOlanders\YOOessentials\Vendor\GuzzleHttp\ClientInterface;
use ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Ring\Client\StreamHandler;
use InvalidArgumentException;
use LogicException;
use ZOOlanders\YOOessentials\Vendor\Monolog\Handler\StreamHandler as MonologStreamHandler;
use ZOOlanders\YOOessentials\Vendor\Monolog\Handler\SyslogHandler as MonologSyslogHandler;
...
class Client
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (!\is_null($this->config['credentials'])) {
            if ($this->config['credentials'] instanceof \ZOOlanders\YOOessentials\Vendor\Google\Auth\CredentialsLoader) {
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \ZOOlanders\YOOessentials\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\ZOOlanders\YOOessentials\Vendor\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \ZOOlanders\YOOessentials\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\ZOOlanders\YOOessentials\Vendor\GuzzleHttp\ClientInterface $http = null)
    {
    ...
    {
        $tokenRevoker = new \ZOOlanders\YOOessentials\Vendor\Google\AccessToken\Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \ZOOlanders\YOOessentials\Vendor\Google\AccessToken\Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
     ...
     */
    public function execute(\ZOOlanders\YOOessentials\Vendor\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \ZOOlanders\YOOessentials\Vendor\Google\Http\REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \ZOOlanders\YOOessentials\Vendor\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
     ...
     */
    public function setCache(\ZOOlanders\YOOessentials\Vendor\Psr\Cache\CacheItemPoolInterface $cache)
    {
        $this->cache = $cache;
    }
    /**
    ...
    {
        $logger = new \ZOOlanders\YOOessentials\Vendor\Monolog\Logger('google-api-php-client');
        if ($this->isAppEngine()) {
            $handler = new \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\SyslogHandler('app', \LOG_USER, \ZOOlanders\YOOessentials\Vendor\Monolog\Logger::NOTICE);
        } else {
            $handler = new \ZOOlanders\YOOessentials\Vendor\Monolog\Handler\StreamHandler('php://stderr', \ZOOlanders\YOOessentials\Vendor\Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \ZOOlanders\YOOessentials\Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
     * Set the Http Client object
     * @param ClientInterface $http
     */
    public function setHttpClient(\ZOOlanders\YOOessentials\Vendor\GuzzleHttp\ClientInterface $http)
    {
        $this->http = $http;
    }
    /**
    ...
    protected function createDefaultHttpClient()
    {
        ...
        if (\defined('\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            $guzzleVersion = \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        } elseif (\defined('\\GuzzleHttp\\ClientInterface::VERSION')) {
            // @phpstan-ignore-next-line
            $guzzleVersion = (int) \substr(\ZOOlanders\YOOessentials\Vendor\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
        if (5 === $guzzleVersion) {
            $options = ['base_url' => $this->config['base_path'], 'defaults' => ['exceptions' => \false]];
            if ($this->isAppEngine()) {
                if (\class_exists(\ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Ring\Client\StreamHandler::class)) {
                    // set StreamHandler on AppEngine by default
                    $options['handler'] = new \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Ring\Client\StreamHandler();
        ...
        return new \ZOOlanders\YOOessentials\Vendor\GuzzleHttp\Client($options);
    }
    /**
    ...
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \ZOOlanders\YOOessentials\Vendor\Google\Auth\CredentialsLoader::makeCredentials($scopes, $serviceAccountCredentials);
        } else {
            ...
            $credentials = \ZOOlanders\YOOessentials\Vendor\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, $sub ? null : $this->config['cache_config'], $sub ? null : $this->getCache(), $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \ZOOlanders\YOOessentials\Vendor\Google\Auth\Credentials\ServiceAccountCredentials) {
        ...
        if (!$credentials instanceof \ZOOlanders\YOOessentials\Vendor\Google\Auth\FetchAuthTokenCache) {
            $credentials = new \ZOOlanders\YOOessentials\Vendor\Google\Auth\FetchAuthTokenCache($credentials, $this->config['cache_config'], $this->getCache());
        }
        return $credentials;
    }
    protected function getAuthHandler()
    {
        ...
        return \ZOOlanders\YOOessentials\Vendor\Google\AuthHandler\AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
    private function createUserRefreshCredentials($scope, $refreshToken)
    {
        $creds = \array_filter(['client_id' => $this->getClientId(), 'client_secret' => $this->getClientSecret(), 'refresh_token' => $refreshToken]);
        return new \ZOOlanders\YOOessentials\Vendor\Google\Auth\Credentials\UserRefreshCredentials($scope, $creds);
    }
}
