 ...
 */
namespace ZOOlanders\YOOessentials\Vendor\Google\Task;

use ZOOlanders\YOOessentials\Vendor\Google\Service\Exception as GoogleServiceException;
use ZOOlanders\YOOessentials\Vendor\Google\Task\Exception as GoogleTaskException;
/**
...
class Runner
{
    ...
    public function __construct($config, $name, $action, array $arguments = [])
    {
        if (isset($config['initial_delay'])) {
            if ($config['initial_delay'] < 0) {
                throw new \ZOOlanders\YOOessentials\Vendor\Google\Task\Exception('Task configuration `initial_delay` must not be negative.');
            }
            $this->delay = $config['initial_delay'];
        }
        if (isset($config['max_delay'])) {
            if ($config['max_delay'] <= 0) {
                throw new \ZOOlanders\YOOessentials\Vendor\Google\Task\Exception('Task configuration `max_delay` must be greater than 0.');
            }
            $this->maxDelay = $config['max_delay'];
        }
        if (isset($config['factor'])) {
            if ($config['factor'] <= 0) {
                throw new \ZOOlanders\YOOessentials\Vendor\Google\Task\Exception('Task configuration `factor` must be greater than 0.');
            }
            $this->factor = $config['factor'];
        }
        if (isset($config['jitter'])) {
            if ($config['jitter'] <= 0) {
                throw new \ZOOlanders\YOOessentials\Vendor\Google\Task\Exception('Task configuration `jitter` must be greater than 0.');
            }
            $this->jitter = $config['jitter'];
        }
        if (isset($config['retries'])) {
            if ($config['retries'] < 0) {
                throw new \ZOOlanders\YOOessentials\Vendor\Google\Task\Exception('Task configuration `retries` must not be negative.');
            }
            $this->maxAttempts += $config['retries'];
        }
        if (!\is_callable($action)) {
            throw new \ZOOlanders\YOOessentials\Vendor\Google\Task\Exception('Task argument `$action` must be a valid callable.');
        }
        $this->action = $action;
    ...
    public function run()
    {
        while ($this->attempt()) {
            ...
            } catch (\ZOOlanders\YOOessentials\Vendor\Google\Service\Exception $exception) {
