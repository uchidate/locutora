...
final class Grantee
{
    ...
    public function requestBody(\DOMElement $node, \DOMDocument $document) : void
    {
        ...
        if (null === ($v = $this->type)) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('Missing parameter "Type" for "%s". The value cannot be null.', __CLASS__));
        }
        if (!\ZOOlanders\YOOessentials\Vendor\AsyncAws\S3\Enum\Type::exists($v)) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('Invalid parameter "xsi:type" for "%s". The value "%s" is not a valid "Type".', __CLASS__, $v));
        }
