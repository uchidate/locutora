...
final class ListObjectsV2Request extends \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Input
{
    /**
     * Bucket name to list.
     *
    ...
    public function request() : \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request
    {
        ...
        if (null !== $this->encodingType) {
            if (!\ZOOlanders\YOOessentials\Vendor\AsyncAws\S3\Enum\EncodingType::exists($this->encodingType)) {
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('Invalid parameter "EncodingType" for "%s". The value "%s" is not a valid "EncodingType".', __CLASS__, $this->encodingType));
            }
