 ...
 */
class ListObjectsV2Output extends \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Result implements \IteratorAggregate
{
    /**
    ...
    public function getCommonPrefixes(bool $currentPageOnly = \false) : iterable
    {
        ...
        if (!$this->input instanceof \ZOOlanders\YOOessentials\Vendor\AsyncAws\S3\Input\ListObjectsV2Request) {
    ...
    private function populateResultObjectList(\SimpleXMLElement $xml) : array
    {
        $items = [];
        foreach ($xml as $item) {
            $items[] = new \ZOOlanders\YOOessentials\Vendor\AsyncAws\S3\ValueObject\AwsObject(['Key' => ($v = $item->Key) ? (string) $v : null, 'LastModified' => ($v = $item->LastModified) ? new \DateTimeImmutable((string) $v) : null, 'ETag' => ($v = $item->ETag) ? (string) $v : null, 'ChecksumAlgorithm' => !$item->ChecksumAlgorithm ? null : $this->populateResultChecksumAlgorithmList($item->ChecksumAlgorithm), 'Size' => ($v = $item->Size) ? (string) $v : null, 'StorageClass' => ($v = $item->StorageClass) ? (string) $v : null, 'Owner' => !$item->Owner ? null : new \ZOOlanders\YOOessentials\Vendor\AsyncAws\S3\ValueObject\Owner(['DisplayName' => ($v = $item->Owner->DisplayName) ? (string) $v : null, 'ID' => ($v = $item->Owner->ID) ? (string) $v : null])]);
        }
        return $items;
    }
}
