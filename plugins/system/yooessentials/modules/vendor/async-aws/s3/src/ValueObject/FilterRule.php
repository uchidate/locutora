...
final class FilterRule
{
    ...
    {
        if (null !== ($v = $this->name)) {
            if (!\ZOOlanders\YOOessentials\Vendor\AsyncAws\S3\Enum\FilterRuleName::exists($v)) {
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('Invalid parameter "Name" for "%s". The value "%s" is not a valid "FilterRuleName".', __CLASS__, $v));
            }
