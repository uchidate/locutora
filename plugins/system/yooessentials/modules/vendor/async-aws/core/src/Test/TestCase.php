...
use ZOOlanders\YOOessentials\Vendor\PHPUnit\Framework\TestCase as PHPUnitTestCase;
...
class TestCase extends \ZOOlanders\YOOessentials\Vendor\PHPUnit\Framework\TestCase
{
     ...
     * Asserts that two Body documents are equal.
     */
    public static function assertRequestEqualsHttpRequest(string $expected, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $actual, string $message = '')
    {
        $expected = \explode("\n\n", \trim($expected));
        $headers = $expected[0];
        $body = $expected[1] ?? '';
        ...
        foreach ($headers as $header) {
            [$key, $value] = \explode(':', \trim($header), 2);
            $expectedHeaders[\strtolower($key)] = \trim($value);
        }
        ...
        switch ($expectedHeaders['content-type'] ?? null) {
            ...
            case 'application/xml':
                ...
                break;
            default:
                self::assertSame(\trim($body), $actual->getBody()->stringify());
                break;
        }
    }
}
if (!\class_exists(\ZOOlanders\YOOessentials\Vendor\PHPUnit\Framework\TestCase::class)) {
    \class_alias(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Test\InternalTestCase::class, \ZOOlanders\YOOessentials\Vendor\PHPUnit\Framework\TestCase::class);
}
