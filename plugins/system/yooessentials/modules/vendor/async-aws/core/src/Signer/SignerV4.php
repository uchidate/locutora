 ...
 */
class SignerV4 implements \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Signer\Signer
{
    ...
    protected function buildBodyDigest(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request, bool $isPresign) : string
    {
        if ($request->hasHeader('x-amz-content-sha256')) {
            ...
            if ($body instanceof \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Stream\ReadOnceResultStream) {
                $request->setBody($body = \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Stream\RewindableStream::create($body));
            }
    ...
    protected function convertBodyToStream(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Signer\SigningContext $context) : void
    {
        $request = $context->getRequest();
        $request->setBody(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Stream\StringStream::create($request->getBody()));
    }
    protected function buildCanonicalPath(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request) : string
    {
        $doubleEncoded = \rawurlencode(\ltrim($request->getUri(), '/'));
        return '/' . \str_replace('%2F', '/', $doubleEncoded);
    }
    private function handleSignature(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Credentials\Credentials $credentials, \DateTimeImmutable $now, \DateTimeImmutable $expires, bool $isPresign) : void
    {
        ...
        $context = new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Signer\SigningContext($request, $now, \implode('/', $credentialScope), $this->buildSigningKey($credentials, $credentialScope));
        if ($isPresign) {
    ...
    private function removePresign(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request) : void
    {
    ...
    private function sanitizeHostForHeader(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request) : void
    {
        if (\false === ($parsedUrl = \parse_url($request->getEndpoint()))) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('The endpoint "%s" is invalid.', $request->getEndpoint()));
        }
    ...
    private function assignAmzQueryValues(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Credentials\Credentials $credentials, bool $isPresign) : void
    {
        if ($isPresign) {
    ...
    private function buildTime(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request, \DateTimeImmutable $now, \DateTimeImmutable $expires, bool $isPresign) : void
    {
        if ($isPresign) {
            $duration = $expires->getTimestamp() - $now->getTimestamp();
            if ($duration > 604800) {
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument('The expiration date of presigned URL must be less than one week');
            }
            if ($duration < 0) {
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument('The expiration date of presigned URL must be in the future');
            }
    ...
    private function buildCredentialString(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Credentials\Credentials $credentials, \DateTimeImmutable $now, bool $isPresign) : array
    {
    ...
    private function convertHeaderToQuery(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request) : void
    {
    ...
    private function convertBodyToQuery(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request) : void
    {
        ...
        $request->setBody(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Stream\StringStream::create(''));
    }
    private function buildCanonicalHeaders(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request, bool $isPresign) : array
    {
    ...
    private function buildCanonicalRequest(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request, array $canonicalHeaders, string $bodyDigest) : string
    {
        return \implode("\n", [
            $request->getMethod(),
    ...
    private function buildCanonicalQuery(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request) : string
    {
    ...
    private function buildSigningKey(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Credentials\Credentials $credentials, array $credentialScope) : string
    {
