...
class Waiter
{
    ...
    public function __construct(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Response $response, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\AbstractApi $awsClient, $request)
    {
    ...
    public final function getState() : string
    {
        ...
        try {
            ...
            $exception = null;
        } catch (\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\Http\HttpException $exception) {
            // use $exception later
        } finally {
            $this->resolved = \true;
        ...
        switch ($state) {
            ...
            case self::STATE_PENDING:
                break;
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('Unexpected state "%s" from Waiter "%s".', $state, __CLASS__));
        }
        return $state;
    }
    /**
