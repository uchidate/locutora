 ...
 */
class ResponseBodyResourceStream implements \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Stream\ResultStream
{
    /**
     * @var resource
     */
    ...
    public function getChunks() : iterable
    {
        ...
        if (0 !== $pos && !\rewind($this->resource)) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\RuntimeException('The stream is not rewindable');
        }
        try {
    ...
    public function getContentAsString() : string
    {
        ...
        try {
            if (!\rewind($this->resource)) {
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\RuntimeException('Failed to rewind the stream');
            }
