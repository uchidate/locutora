...
abstract class AbstractApi
{
    ...
    public function __construct($configuration = [], ?\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Credentials\CredentialProvider $credentialProvider = null, ?\ZOOlanders\YOOessentials\Vendor\Symfony\Contracts\HttpClient\HttpClientInterface $httpClient = null, ?\ZOOlanders\YOOessentials\Vendor\Psr\Log\LoggerInterface $logger = null)
    {
        if (\is_array($configuration)) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('First argument to "%s::__construct()" must be an array or an instance of "%s"', static::class, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Configuration::class));
        }
        $this->logger = $logger ?? new \ZOOlanders\YOOessentials\Vendor\Psr\Log\NullLogger();
        $this->awsErrorFactory = $this->getAwsErrorFactory();
        $this->endpointCache = new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\EndpointDiscovery\EndpointCache();
        if (!isset($httpClient)) {
            ...
            if (\class_exists(\ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\RetryableHttpClient::class)) {
                ...
                $httpClient = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\RetryableHttpClient($httpClient, new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\HttpClient\AwsRetryStrategy(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\HttpClient\AwsRetryStrategy::DEFAULT_RETRY_STATUS_CODES, 1000, 2.0, 0, 0.1, $this->awsErrorFactory), 3, $this->logger);
            }
        }
    ...
    public final function getConfiguration() : \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Configuration
    {
        return $this->configuration;
    }
    public final function presign(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Input $input, ?\DateTimeImmutable $expires = null) : string
    {
        ...
        if (null !== ($credentials = $this->credentialProvider->getCredentials($this->configuration))) {
            $this->getSigner($input->getRegion())->presign($request, $credentials, new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\RequestContext(['expirationDate' => $expires]));
        }
    ...
    {
        throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('The method "%s" should not be called. The Client "%s" must implement the "%s" method.', __FUNCTION__, \get_class($this), 'getEndpointMetadata'));
    }
    /**
     * @deprecated
     */
    ...
    protected final function getResponse(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Request $request, ?\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\RequestContext $context = null) : \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Response
    {
        ...
        if (null !== ($credentials = $this->credentialProvider->getCredentials($this->configuration))) {
            $this->getSigner($context ? $context->getRegion() : null)->sign($request, $credentials, $context ?? new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\RequestContext());
        }
        ...
        if (($requestBody = $request->getBody()) instanceof \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Stream\StringStream) {
        ...
        return new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Response($response, $this->httpClient, $this->logger, $this->awsErrorFactory, $this->endpointCache, $request, $debug, $context ? $context->getExceptionMapping() : []);
    }
    /**
     * @return callable[]
     */
    protected function getSignerFactories() : array
    {
        return ['v4' => static function (string $service, string $region) {
            return new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Signer\SignerV4($service, $region);
        }];
    }
    protected function getAwsErrorFactory() : \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\AwsError\AwsErrorFactoryInterface
    {
        return new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\AwsError\ChainAwsErrorFactory();
    }
    /**
    ...
    {
        throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('The Client "%s" must implement the "%s" method.', \get_class($this), 'discoverEndpoints'));
    }
    private function getDiscoveredEndpoint(string $uri, array $query, ?string $region, bool $usesEndpointDiscovery, bool $requiresEndpointDiscovery)
    {
        ...
        $usesEndpointDiscovery = $requiresEndpointDiscovery || $usesEndpointDiscovery && \filter_var($this->configuration->get(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Configuration::OPTION_ENDPOINT_DISCOVERY_ENABLED), \FILTER_VALIDATE_BOOLEAN);
        ...
        if (null === ($endpoint = $this->endpointCache->getActiveEndpoint($region))) {
            ...
            if (null === $endpoint && null === ($endpoint = $this->endpointCache->getExpiredEndpoint($region))) {
                if ($requiresEndpointDiscovery) {
                    throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\RuntimeException(\sprintf('The Client "%s" failed to fetch the endpoint.', \get_class($this)), 0, $previous);
                }
                return $this->getEndpoint($uri, $query, $region);
            }
        }
    ...
    private function getSigner(?string $region)
    {
        ...
        if (!isset($this->signers[$region])) {
            ...
            if ($this->configuration->isDefault('endpoint') || $this->configuration->isDefault('region')) {
                ...
                $metadata = $this->getEndpointMetadata(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Configuration::DEFAULT_REGION);
            ...
            if (null === $factory) {
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('None of the signatures "%s" is implemented.', \implode(', ', $metadata['signVersions'])));
            }
