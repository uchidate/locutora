...
namespace ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Test;
...
use ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Test\Http\SimpleMockedResponse;
use ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Waiter;
use ZOOlanders\YOOessentials\Vendor\Psr\Log\NullLogger;
use ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\MockHttpClient;
/**
...
class ResultMockFactory
{
     ...
     * @return Result|T
     */
    public static function createFailing(string $class, int $code, ?string $message = null, array $additionalContent = [])
    {
        if (\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Result::class !== $class) {
            $parent = \get_parent_class($class);
            if (\false === $parent || \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Result::class !== $parent) {
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('The "%s::%s" can only be used for classes that extend "%s"', __CLASS__, __METHOD__, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Result::class));
            }
        }
        $httpResponse = new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Test\Http\SimpleMockedResponse(\json_encode(\array_merge(['message' => $message], $additionalContent)), ['content-type' => 'application/json'], $code);
        $client = new \ZOOlanders\YOOessentials\Vendor\Symfony\Component\HttpClient\MockHttpClient($httpResponse);
        $response = new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Response($client->request('POST', 'http://localhost'), $client, new \ZOOlanders\YOOessentials\Vendor\Psr\Log\NullLogger());
    ...
    public static function create(string $class, array $data = [])
    {
        ...
        $reflectionClass = new \ReflectionClass(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Result::class);
        $initializedProperty = $reflectionClass->getProperty('initialized');
        $initializedProperty->setAccessible(\true);
        $reflectionClass = new \ReflectionClass($class);
        $object = $reflectionClass->newInstance($response);
        if (\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Result::class !== $class) {
            self::addPropertiesOnResult($reflectionClass, $object, $class);
        }
        $initializedProperty->setValue($object, \true);
    ...
    public static function waiter(string $class, string $finalState)
    {
        if (\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Result::class !== $class) {
            $parent = \get_parent_class($class);
            if (\false === $parent || \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Waiter::class !== $parent) {
                throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('The "%s::%s" can only be used for classes that extend "%s"', __CLASS__, __METHOD__, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Waiter::class));
            }
        }
        if (\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Waiter::STATE_SUCCESS !== $finalState && \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Waiter::STATE_FAILURE !== $finalState) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('The state passed to "%s::%s" must be "%s" or "%s".', __CLASS__, __METHOD__, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Waiter::STATE_SUCCESS, \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Waiter::STATE_FAILURE));
        }
        $response = self::getResponseObject();
        $reflectionClass = new \ReflectionClass(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Waiter::class);
        $propertyResponse = $reflectionClass->getProperty('response');
        $propertyResponse->setAccessible(\true);
        $propertyState = $reflectionClass->getProperty('finalState');
        $propertyState->setAccessible(\true);
        $reflectionClass = new \ReflectionClass($class);
    ...
    private static function addPropertiesOnResult(\ReflectionClass $reflectionClass, object $object, string $class) : void
    {
        if (\false === ($pos = \strrpos($class, '\\'))) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('Expected class "%s" to have a backslash. ', $class));
        }
        ...
        if ('Output' === \substr($className, -6)) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('Unknown class suffix: "%s"', $className));
        }
        if (\false === ($pos = \strrpos($class, '\\', -2 - \strlen($className)))) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('Expected class "%s" to have more than one backslash. ', $class));
        }
        $baseNamespace = \substr($class, 0, $pos);
        if (\false === ($pos = \strrpos($baseNamespace, '\\'))) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\LogicException(\sprintf('Expected base namespace "%s" to have a backslash. ', $baseNamespace));
        }
    ...
    private static function getResponseObject() : \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Response
    {
        $reflectionClass = new \ReflectionClass(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Response::class);
        $response = $reflectionClass->newInstanceWithoutConstructor();
        $property = $reflectionClass->getProperty('resolveResult');
        $property->setAccessible(\true);
        $property->setValue($response, \true);
