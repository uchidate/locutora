...
final class Configuration
{
    ...
    public static function create(array $options)
    {
        if (0 < \count($invalidOptions = \array_diff_key($options, self::AVAILABLE_OPTIONS))) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('Invalid option(s) "%s" passed to "%s::%s". ', \implode('", "', \array_keys($invalidOptions)), __CLASS__, __METHOD__));
        }
    ...
    public function get(string $name) : ?string
    {
        if (!isset(self::AVAILABLE_OPTIONS[$name])) {
            throw new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Exception\InvalidArgument(\sprintf('Invalid option "%s" passed to "%s::%s". ', $name, __CLASS__, __METHOD__));
        }
        ...
        foreach (self::FALLBACK_OPTIONS as $fallbackGroup) {
            ...
            foreach ($fallbackGroup as $option => $envVariableNames) {
                ...
                foreach ($envVariableNames as $envVariableName) {
                    if (null !== ($envVariableValue = \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\EnvVar::get($envVariableName))) {
     ...
     */
    private static function parseIniFiles(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Configuration $configuration) : array
    {
        $options = [];
        if (!$configuration->isDefault(self::OPTION_REGION)) {
            return $options;
        }
        $profilesData = (new \ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Credentials\IniFileLoader())->loadProfiles([$configuration->get(self::OPTION_SHARED_CREDENTIALS_FILE), $configuration->get(self::OPTION_SHARED_CONFIG_FILE)]);
     ...
     */
    private static function populateConfiguration(\ZOOlanders\YOOessentials\Vendor\AsyncAws\Core\Configuration $configuration, array $options) : void
    {
