...
class Decoder
{
    ...
    {
        $ctrlByte = \ord(\ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, 1));
        ++$offset;
        $type = $ctrlByte >> 5;
        ...
        if ($type === self::_EXTENDED) {
            $nextByte = \ord(\ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, 1));
            $type = $nextByte + 7;
            if ($type < 8) {
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\InvalidDatabaseException('Something went horribly wrong in the decoder. An extended type ' . 'resolved to a type number < 8 (' . $type . ')');
            }
            ++$offset;
        }
    ...
    private function decodeByType(int $type, int $offset, int $size) : array
    {
        ...
        $bytes = \ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, $size);
        switch ($type) {
            case self::_BYTES:
            ...
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\InvalidDatabaseException('Unknown or unexpected type: ' . $type);
        }
    }
    private function verifySize(int $expected, int $actual) : void
    {
        if ($expected !== $actual) {
            throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\InvalidDatabaseException("The MaxMind DB file's data section contains bad data (unknown data type or corrupt data)");
        }
    }
    ...
    private function decodePointer(int $ctrlByte, int $offset) : array
    {
        ...
        $buffer = \ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, $pointerSize);
        ...
        switch ($pointerSize) {
            ...
            case 4:
                ...
                break;
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\InvalidDatabaseException('Unexpected pointer size ' . $pointerSize);
        }
    ...
    private function sizeFromCtrlByte(int $ctrlByte, int $offset) : array
    {
        ...
        $bytesToRead = $size - 28;
        $bytes = \ZOOlanders\YOOessentials\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, $bytesToRead);
        if ($size === 29) {
            $size = 29 + \ord($bytes);
        } elseif ($size === 30) {
