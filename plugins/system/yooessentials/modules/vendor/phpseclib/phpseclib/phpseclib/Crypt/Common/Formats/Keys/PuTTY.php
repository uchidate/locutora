...
abstract class PuTTY
{
    ...
    private static function generateV2Key($password, $length)
    {
        ...
        while (\strlen($symkey) < $length) {
            ...
            $symkey .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::hex2bin(\sha1($temp));
        }
    ...
    private static function generateV3Key($password, $flavour, $memory, $passes, $salt)
    {
        ...
        switch ($flavour) {
            ...
            case 'Argon2id':
                ...
                break;
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('Only Argon2i and Argon2id are supported');
        }
        $length = 80;
    ...
    public static function load($key, $password)
    {
        ...
        $components = [];
        $key = \preg_split('#\\r\\n|\\r|\\n#', \trim($key));
        if (\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($key[0], \strlen('PuTTY-User-Key-File-')) != 'PuTTY-User-Key-File-') {
            return \false;
        }
        $version = (int) \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($key[0], 3);
        ...
        if (!\in_array($type, static::$types)) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException($error . 'This is an unsupported ' . $type . ' key');
        }
        ...
        $public = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_decode(\implode('', \array_map('trim', \array_slice($key, 4, $publicLength))));
        $source = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('ssss', $type, $encryption, $components['comment'], $public);
        \extract(\unpack('Nlength', \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($public, 4)));
        $newtype = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($public, $length);
        if ($newtype != $type) {
        ...
        switch ($encryption) {
            case 'aes256-cbc':
                $crypto = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\AES('cbc');
                switch ($version) {
                    case 3:
                        ...
                        $salt = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::hex2bin(\trim(\preg_replace('#Argon2-Salt: ([0-9a-f]+)#', '$1', $key[$offset++])));
        ...
        switch ($version) {
            case 3:
                $hash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha256');
                $hash->setKey($hashkey);
                break;
            case 2:
        ...
        $private = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_decode(\implode('', \array_map('trim', \array_slice($key, $offset, $privateLength))));
        if ($encryption != 'none') {
        ...
        $source .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s', $private);
        $hmac = \trim(\preg_replace('#Private-MAC: (.+)#', '$1', $key[$offset + $privateLength]));
        $hmac = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::hex2bin($hmac);
    ...
    protected static function wrapPrivateKey($public, $private, $type, $password, array $options = [])
    {
        ...
        $key .= "Comment: {$comment}\r\n";
        $public = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s', $type) . $public;
        $source = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('ssss', $type, $encryption, $comment, $public);
        $public = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($public);
        ...
        if (empty($password) && !\is_string($password)) {
            ...
            $private .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random::string(16 - (\strlen($private) & 15));
            $source .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s', $private);
            $crypto = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\AES('cbc');
            switch ($version) {
                case 3:
                    $salt = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random::string(16);
                    $key .= "Key-Derivation: Argon2id\r\n";
                    $key .= "Argon2-Memory: 8192\r\n";
                    $key .= "Argon2-Passes: 13\r\n";
                    $key .= "Argon2-Parallelism: 1\r\n";
                    $key .= "Argon2-Salt: " . \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($salt) . "\r\n";
        ...
        $private = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($private);
        $key .= 'Private-Lines: ' . (\strlen($private) + 63 >> 6) . "\r\n";
        $key .= \chunk_split($private, 64);
        $key .= 'Private-MAC: ' . \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($hash->hash($source)) . "\r\n";
        return $key;
    }
    /**
    ...
    protected static function wrapPublicKey($key, $type)
    {
        ...
        $key = "---- BEGIN SSH2 PUBLIC KEY ----\r\n" . 'Comment: "' . \str_replace(['\\', '"'], ['\\\\', '\\"'], self::$comment) . "\"\r\n" . \chunk_split(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($key), 64) . '---- END SSH2 PUBLIC KEY ----';
        return $key;
    }
}
