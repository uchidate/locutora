 ...
 */
namespace ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA;
...
use ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\DigestInfo;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class PublicKey extends \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA implements \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PublicKey
{
     ...
     */
    private function exponentiate(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger $x)
    {
    ...
    private function rsassa_pkcs1_v1_5_verify($m, $s)
    {
        ...
        try {
            ...
            $exception = \true;
        } catch (\ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException $e) {
            $r2 = \false;
        }
        if ($exception) {
    ...
    private function rsassa_pkcs1_v1_5_relaxed_verify($m, $s)
    {
        ...
        if (\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($em, 2) != "\x00\x01") {
            return \false;
        }
        $em = \ltrim($em, "\xff");
        if (\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($em) != "\x00") {
            return \false;
        }
        $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($em);
        ...
        if (!isset($oids)) {
            ...
            \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::loadOIDs($oids);
        }
        $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\DigestInfo::MAP);
        ...
        $hash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash($hash);
        $em = $hash->hash($m);
        $em2 = $decoded['digest'];
    ...
    private function rsaes_pkcs1_v1_5_encrypt($m, $pkcs15_compat = \false)
    {
        ...
        while (\strlen($ps) != $psLen) {
            $temp = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random::string($psLen - \strlen($ps));
    ...
    private function rsaes_oaep_encrypt($m)
    {
        ...
        $db = $lHash . $ps . \chr(1) . $m;
        $seed = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random::string($this->hLen);
    ...
    public function toString($type, array $options = [])
    {
        ...
        if ($type == \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS::class) {
            if ($this->signaturePadding == self::SIGNATURE_PSS) {
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedFormatException('The PSS format can only be used when the signature method has been explicitly set to PSS');
            }
        }
    ...
    {
        $new = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\PrivateKey();
