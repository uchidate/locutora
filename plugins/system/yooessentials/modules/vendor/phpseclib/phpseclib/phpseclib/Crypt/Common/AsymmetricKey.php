...
abstract class AsymmetricKey
{
    ...
    protected function __construct()
    {
        ...
        $this->hash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha256');
        $this->hmac = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha256');
    }
    /**
    ...
    {
        if (!isset(self::$zero)) {
            self::$zero = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(0);
            self::$one = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(1);
        }
    ...
    public static function load($key, $password = \false)
    {
        ...
        $new->format = $format;
        $new->comment = $comment;
        return $new instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PrivateKey ? $new->withPassword($password) : $new;
    }
    /**
     * Loads a private key
     *
    ...
    protected static function validatePlugin($format, $type, $method = null)
    {
        ...
        if (!isset(self::$plugins[static::ALGORITHM][$format][$type])) {
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedFormatException("{$type} is not a supported format");
        }
        $type = self::$plugins[static::ALGORITHM][$format][$type];
        if (isset($method) && !\method_exists($type, $method)) {
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedFormatException("{$type} does not implement {$method}");
        }
        return $type;
    }
    /**
    ...
    public function getLoadedFormat()
    {
        if (empty($this->format)) {
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoKeyLoadedException('This key was created with createKey - it was not loaded with load. Therefore there is no "loaded format"');
        }
    ...
    public function withHash($hash)
    {
        ...
        $new->hmac = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash($hash);
        return $new;
    }
    /**
    ...
    {
        $v = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger($in, 256);
        $vlen = \strlen($in) << 3;
