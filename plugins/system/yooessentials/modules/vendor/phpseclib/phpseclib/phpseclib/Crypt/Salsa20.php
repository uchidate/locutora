...
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\StreamCipher;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class Salsa20 extends \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\StreamCipher
{
    /**
     * Part 1 of the state
     *
    ...
    private function crypt($text, $mode)
    {
        ...
        if (!\strlen($buffer['ciphertext'])) {
            ...
            $ciphertext = $text ^ \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($buffer['ciphertext'], \strlen($text));
        ...
        // & 0x3F
        if ($overflow) {
            $text2 = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::pop($text, $overflow);
            if ($this->engine == self::ENGINE_OPENSSL) {
                ...
                $temp = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::pop($encrypted, 64);
            } else {
                $blocks = \str_split($text, 64);
                if (\strlen($text)) {
                    foreach ($blocks as &$block) {
