 ...
 */
namespace ZOOlanders\YOOessentials\Vendor\phpseclib3\Net;

use ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Blowfish;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\ChaCha20;
...
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\SymmetricKey;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RC4;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Rijndael;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\TripleDES;
// Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Twofish;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\ConnectionClosedException;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\InsufficientSetupException;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnableToConnectException;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException;
...
use ZOOlanders\YOOessentials\Vendor\phpseclib3\System\SSH\Agent;
/**
 * Pure-PHP implementation of SSHv2.
 *
 * @author  Jim Wigginton <terrafrost@php.net>
...
class SSH2
{
    ...
    public function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        if (\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::is_stringable($host)) {
            $this->host = $host;
            $this->port = $port;
    ...
    private function connect()
    {
        ...
        if (!\is_resource($this->fsock)) {
            ...
            if (!$this->fsock) {
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnableToConnectException(\rtrim("Cannot connect to {$host}. Error {$errno}. {$errstr}"));
            }
        ...
        if (\feof($this->fsock)) {
            $this->bitmap = 0;
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\ConnectionClosedException('Connection closed by server');
        }
        $extra = $matches[1];
        ...
        if (\version_compare($matches[3], '1.99', '<')) {
            $this->bitmap = 0;
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnableToConnectException("Cannot connect to SSH {$matches[3]} servers");
        }
        if (!$this->send_id_string_first) {
            \fputs($this->fsock, $this->identifier . "\r\n");
        }
    ...
    private function key_exchange($kexinit_payload_server = \false)
    {
        ...
        $send_kex = \true;
        $kex_algorithms = isset($preferred['kex']) ? $preferred['kex'] : \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::getSupportedKEXAlgorithms();
        $server_host_key_algorithms = isset($preferred['hostkey']) ? $preferred['hostkey'] : \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::getSupportedHostKeyAlgorithms();
        $s2c_encryption_algorithms = isset($preferred['server_to_client']['crypt']) ? $preferred['server_to_client']['crypt'] : \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::getSupportedEncryptionAlgorithms();
        $c2s_encryption_algorithms = isset($preferred['client_to_server']['crypt']) ? $preferred['client_to_server']['crypt'] : \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::getSupportedEncryptionAlgorithms();
        $s2c_mac_algorithms = isset($preferred['server_to_client']['mac']) ? $preferred['server_to_client']['mac'] : \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::getSupportedMACAlgorithms();
        $c2s_mac_algorithms = isset($preferred['client_to_server']['mac']) ? $preferred['client_to_server']['mac'] : \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::getSupportedMACAlgorithms();
        $s2c_compression_algorithms = isset($preferred['server_to_client']['comp']) ? $preferred['server_to_client']['comp'] : \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::getSupportedCompressionAlgorithms();
        $c2s_compression_algorithms = isset($preferred['client_to_server']['comp']) ? $preferred['client_to_server']['comp'] : \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::getSupportedCompressionAlgorithms();
        ...
        $client_cookie = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random::string(16);
        $kexinit_payload_client = \pack('Ca*', NET_SSH2_MSG_KEXINIT, $client_cookie);
        $kexinit_payload_client .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2(
            'L10bN',
            $kex_algorithms,
        ...
        \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($response, 1);
        // skip past the message number (it should be SSH_MSG_KEXINIT)
        $server_cookie = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($response, 16);
        list($this->kex_algorithms, $this->server_host_key_algorithms, $this->encryption_algorithms_client_to_server, $this->encryption_algorithms_server_to_client, $this->mac_algorithms_client_to_server, $this->mac_algorithms_server_to_client, $this->compression_algorithms_client_to_server, $this->compression_algorithms_server_to_client, $this->languages_client_to_server, $this->languages_server_to_client, $first_kex_packet_follows) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('L10C', $response);
        if ($send_kex) {
            $this->send_binary_packet($kexinit_payload_client);
        }
        ...
        if ($decryptKeyLength === null) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible server to client encryption algorithms found');
        }
        ...
        if ($encryptKeyLength === null) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible client to server encryption algorithms found');
        }
        // through diffie-hellman key exchange a symmetric key is obtained
        $this->kex_algorithm = self::array_intersect_first($kex_algorithms, $this->kex_algorithms);
        if ($this->kex_algorithm === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible key exchange algorithms found');
        }
        $server_host_key_algorithm = self::array_intersect_first($server_host_key_algorithms, $this->server_host_key_algorithms);
        if ($server_host_key_algorithm === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible server host key algorithms found');
        }
        $mac_algorithm_out = self::array_intersect_first($c2s_mac_algorithms, $this->mac_algorithms_client_to_server);
        if ($mac_algorithm_out === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible client to server message authentication algorithms found');
        }
        $mac_algorithm_in = self::array_intersect_first($s2c_mac_algorithms, $this->mac_algorithms_server_to_client);
        if ($mac_algorithm_in === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible server to client message authentication algorithms found');
        }
        ...
        if ($compression_algorithm_in === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible server to client compression algorithms found');
        }
        ...
        if ($compression_algorithm_out === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible client to server compression algorithms found');
        }
        ...
        switch ($this->kex_algorithm) {
            ...
            case 'ecdh-sha2-nistp521':
                $kexHash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha512');
                break;
            case 'ecdh-sha2-nistp384':
                $kexHash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha384');
                break;
            ...
            case 'curve25519-sha256':
                $kexHash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha256');
                break;
            default:
                $kexHash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha1');
        }
        ...
        if (\strpos($this->kex_algorithm, 'curve25519-sha256') === 0 || \strpos($this->kex_algorithm, 'ecdh-sha2-nistp') === 0) {
            ...
            $ourPrivate = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC::createKey($curve);
            ...
            if (\strpos($this->kex_algorithm, 'diffie-hellman-group-exchange') === 0) {
                ...
                $response = $this->get_binary_packet();
                list($type, $primeBytes, $gBytes) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('Css', $response);
                ...
                $prime = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger($primeBytes, -256);
                $g = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger($gBytes, -256);
                $exchange_hash_rfc4419 = $dh_group_sizes_packed . \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('ss', $primeBytes, $gBytes);
                $params = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH::createParameters($prime, $g);
                $clientKexInitMessage = 'NET_SSH2_MSG_KEXDH_GEX_INIT';
                $serverKexReplyMessage = 'NET_SSH2_MSG_KEXDH_GEX_REPLY';
            } else {
                $params = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH::createParameters($this->kex_algorithm);
                $clientKexInitMessage = 'NET_SSH2_MSG_KEXDH_INIT';
                $serverKexReplyMessage = 'NET_SSH2_MSG_KEXDH_REPLY';
            }
            $keyLength = \min($kexHash->getLengthInBytes(), \max($encryptKeyLength, $decryptKeyLength));
            $ourPrivate = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH::createKey($params, 16 * $keyLength);
            // 2 * 8 * $keyLength
        ...
        $response = $this->get_binary_packet();
        list($type, $server_public_host_key, $theirPublicBytes, $this->signature) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('Csss', $response);
        ...
        list($public_key_format) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $server_public_host_key);
        ...
        $keyBytes = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH::computeSecret($ourPrivate, $theirPublicBytes);
        if (($keyBytes & "\xff\x80") === "\x00\x00") {
            $keyBytes = \substr($keyBytes, 1);
        } elseif (($keyBytes[0] & "\x80") === "\x80") {
            $keyBytes = "\x00{$keyBytes}";
        }
        $this->exchange_hash = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s5', $this->identifier, $this->server_identifier, $kexinit_payload_client, $kexinit_payload_server, $this->server_public_host_key);
        $this->exchange_hash .= $exchange_hash_rfc4419;
        $this->exchange_hash .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s3', $ourPublicBytes, $theirPublicBytes, $keyBytes);
        ...
        list($type) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('C', $response);
        ...
        if ($this->hmac_create instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash) {
        ...
        if ($this->hmac_check instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash) {
    ...
    private static function encryption_algorithm_to_crypt_instance($algorithm)
    {
        switch ($algorithm) {
            case '3des-cbc':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\TripleDES('cbc');
            case '3des-ctr':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\TripleDES('ctr');
            case 'aes256-cbc':
            case 'aes192-cbc':
            case 'aes128-cbc':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Rijndael('cbc');
            case 'aes256-ctr':
            case 'aes192-ctr':
            case 'aes128-ctr':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Rijndael('ctr');
            case 'blowfish-cbc':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Blowfish('cbc');
            case 'blowfish-ctr':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Blowfish('ctr');
            case 'twofish128-cbc':
            case 'twofish192-cbc':
            case 'twofish256-cbc':
            case 'twofish-cbc':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Twofish('cbc');
            case 'twofish128-ctr':
            case 'twofish192-ctr':
            case 'twofish256-ctr':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Twofish('ctr');
            case 'arcfour':
            case 'arcfour128':
            case 'arcfour256':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RC4();
            case 'aes128-gcm@openssh.com':
            case 'aes256-gcm@openssh.com':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Rijndael('gcm');
            case 'chacha20-poly1305@openssh.com':
                return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\ChaCha20();
        }
        return null;
    }
    /**
    ...
    private static function mac_algorithm_to_hash_instance($algorithm)
    {
        switch ($algorithm) {
            ...
            case 'umac-64-etm@openssh.com':
                return [new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('umac-64'), 16];
            case 'umac-128@openssh.com':
            case 'umac-128-etm@openssh.com':
                return [new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('umac-128'), 16];
            case 'hmac-sha2-512':
            case 'hmac-sha2-512-etm@openssh.com':
                return [new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha512'), 64];
            case 'hmac-sha2-256':
            case 'hmac-sha2-256-etm@openssh.com':
                return [new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha256'), 32];
            case 'hmac-sha1':
            case 'hmac-sha1-etm@openssh.com':
                return [new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha1'), 20];
            case 'hmac-sha1-96':
                return [new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha1-96'), 20];
            case 'hmac-md5':
                return [new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('md5'), 16];
            case 'hmac-md5-96':
                return [new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('md5-96'), 16];
        }
    }
    /*
    ...
    protected function sublogin($username, ...$args)
    {
        ...
        foreach ($args as $arg) {
            switch (\true) {
                case $arg instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PublicKey:
                    throw new \UnexpectedValueException('A PublicKey object was passed to the login method instead of a PrivateKey object');
                case $arg instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PrivateKey:
                case $arg instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\System\SSH\Agent:
                case \is_array($arg):
                case \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::is_stringable($arg):
                    break;
                default:
                    ...
                    switch ($method) {
                        case 'publickey':
                            foreach ($args as $key => $arg) {
                                if ($arg instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PrivateKey || $arg instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\System\SSH\Agent) {
                                    $newargs[] = $arg;
                                    unset($args[$key]);
                                    break;
                                }
                        ...
                        case 'keyboard-interactive':
                            ...
                            foreach ($args as $arg) {
                                ...
                                if ($hasString || \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::is_stringable($arg)) {
                                    $hasString = \true;
                                    break;
                                }
                            }
    ...
    private function login_helper($username, $password = null)
    {
        ...
        if (!($this->bitmap & self::MASK_LOGIN_REQ)) {
            $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('Cs', NET_SSH2_MSG_SERVICE_REQUEST, 'ssh-userauth');
            $this->send_binary_packet($packet);
            try {
                $response = $this->get_binary_packet();
            } catch (\Exception $e) {
            ...
            list($type, $service) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('Cs', $response);
        ...
        if (\strlen($this->last_interactive_response)) {
            return !\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::is_stringable($password) && !\is_array($password) ? \false : $this->keyboard_interactive_process($password);
        }
        if ($password instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PrivateKey) {
            return $this->privatekey_login($username, $password);
        }
        if ($password instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\System\SSH\Agent) {
        ...
        if (!isset($password)) {
            $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('Cs3', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'none');
            $this->send_binary_packet($packet);
            $response = $this->get_binary_packet();
            list($type) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('C', $response);
            switch ($type) {
                ...
                case NET_SSH2_MSG_USERAUTH_FAILURE:
                    list($auth_methods) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('L', $response);
        ...
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('Cs3bs', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'password', \false, $password);
        // remove the username and password from the logged packet
        if (!\defined('ZOOlanders\\YOOessentials\\Vendor\\NET_SSH2_LOGGING')) {
            $logged = null;
        } else {
            $logged = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('Cs3bs', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'password', \false, 'password');
        }
        ...
        switch ($type) {
            case NET_SSH2_MSG_USERAUTH_PASSWD_CHANGEREQ:
                ...
                list($message) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
            ...
            case NET_SSH2_MSG_USERAUTH_FAILURE:
                ...
                list($auth_methods, $partial_success) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('Lb', $response);
                $this->auth_methods_to_continue = $auth_methods;
    ...
    {
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2(
            'Cs5',
    ...
    private function keyboard_interactive_process(...$responses)
    {
        ...
        switch ($type) {
            case NET_SSH2_MSG_USERAUTH_INFO_REQUEST:
                list(, , , $num_prompts) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s3N', $response);
                ...
                if (isset($this->keyboard_requests_responses)) {
                    for ($i = 0; $i < $num_prompts; $i++) {
                        list($prompt, ) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('sC', $response);
                ...
                */
                ...
                for ($i = 0; $i < \count($responses); $i++) {
                    $packet .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s', $responses[$i]);
                    $logged .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s', 'dummy-answer');
                }
                $this->send_binary_packet($packet, $logged);
     ...
     * @return bool
     */
    private function ssh_agent_login($username, \ZOOlanders\YOOessentials\Vendor\phpseclib3\System\SSH\Agent $agent)
    {
        $this->agent = $agent;
     ...
     * @throws \RuntimeException on connection error
     */
    private function privatekey_login($username, \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PrivateKey $privatekey)
    {
        $publickey = $privatekey->getPublicKey();
        if ($publickey instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA) {
            $privatekey = $privatekey->withPadding(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
        ...
        } elseif ($publickey instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC) {
            ...
            switch ($curveName) {
                ...
                default:
                    if (\is_array($curveName)) {
                        throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedCurveException('Specified Curves are not supported by SSH2');
                    }
                    throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedCurveException('Named Curve of ' . $curveName . ' is not supported by phpseclib3\'s SSH2 implementation');
            }
        } elseif ($publickey instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA) {
            $privatekey = $privatekey->withSignatureFormat('SSH2');
            $hash = 'sha1';
            $signatureType = 'ssh-dss';
        } else {
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('Please use either an RSA key, an EC one or a DSA key');
        }
        $publickeyStr = $publickey->toString('OpenSSH', ['binary' => \true]);
        $part1 = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('Csss', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'publickey');
        $part2 = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('ss', $signatureType, $publickeyStr);
        ...
        switch ($type) {
            ...
            default:
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\ConnectionClosedException('Unexpected response to publickey authentication pt 1');
        }
        $packet = $part1 . \chr(1) . $part2;
        $privatekey = $privatekey->withHash($hash);
        $signature = $privatekey->sign(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s', $this->session_id) . $packet);
        if ($publickey instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA) {
            $signature = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('ss', $signatureType, $signature);
        }
        $packet .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s', $signature);
        $this->send_binary_packet($packet);
        $response = $this->get_binary_packet();
        list($type) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('C', $response);
        switch ($type) {
        ...
        $this->disconnect_helper(NET_SSH2_DISCONNECT_BY_APPLICATION);
        throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\ConnectionClosedException('Unexpected response to publickey authentication pt 2');
    }
    /**
     * Set Timeout
     *
    ...
    public function exec($command, callable $callback = null)
    {
        ...
        $packet_size = 0x4000;
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CsN3', NET_SSH2_MSG_CHANNEL_OPEN, 'session', self::CHANNEL_EXEC, $this->window_size_server_to_client[self::CHANNEL_EXEC], $packet_size);
        $this->send_binary_packet($packet);
        $this->channel_status[self::CHANNEL_EXEC] = NET_SSH2_MSG_CHANNEL_OPEN;
        $this->get_channel_packet(self::CHANNEL_EXEC);
        if ($this->request_pty === \true) {
            $terminal_modes = \pack('C', NET_SSH2_TTY_OP_END);
            $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CNsCsN4s', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_EXEC], 'pty-req', 1, $this->term, $this->windowColumns, $this->windowRows, 0, 0, $terminal_modes);
            $this->send_binary_packet($packet);
        ...
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CNsCs', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_EXEC], 'exec', 1, $command);
        $this->send_binary_packet($packet);
        $this->channel_status[self::CHANNEL_EXEC] = NET_SSH2_MSG_CHANNEL_REQUEST;
        if (!$this->get_channel_packet(self::CHANNEL_EXEC)) {
            return \false;
    ...
    private function initShell()
    {
        ...
        $packet_size = 0x4000;
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CsN3', NET_SSH2_MSG_CHANNEL_OPEN, 'session', self::CHANNEL_SHELL, $this->window_size_server_to_client[self::CHANNEL_SHELL], $packet_size);
        $this->send_binary_packet($packet);
        ...
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CNsb', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_SHELL], 'shell', \true);
        $this->send_binary_packet($packet);
    ...
    public function requestAgentForwarding()
    {
        ...
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CNsC', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[$request_channel], 'auth-agent-req@openssh.com', 1);
    ...
    public function read($expect = '', $mode = self::READ_SIMPLE)
    {
        ...
        $this->is_timeout = \false;
        if (!$this->isAuthenticated()) {
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\InsufficientSetupException('Operation disallowed prior to login()');
        }
        ...
        while (\true) {
            ...
            if ($pos !== \false) {
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($this->interactiveBuffer, $pos + \strlen($match));
            }
            $response = $this->get_channel_packet($channel);
            if ($response === \true) {
                $this->in_request_pty_exec = \false;
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($this->interactiveBuffer, \strlen($this->interactiveBuffer));
            }
    ...
    public function startSubsystem($subsystem)
    {
        ...
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CsN3', NET_SSH2_MSG_CHANNEL_OPEN, 'session', self::CHANNEL_SUBSYSTEM, $this->window_size, 0x4000);
        $this->send_binary_packet($packet);
        $this->channel_status[self::CHANNEL_SUBSYSTEM] = NET_SSH2_MSG_CHANNEL_OPEN;
        $this->get_channel_packet(self::CHANNEL_SUBSYSTEM);
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CNsCs', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_SUBSYSTEM], 'subsystem', 1, $subsystem);
        $this->send_binary_packet($packet);
    ...
    public function ping()
    {
        ...
        $packet_size = 0x4000;
        $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CsN3', NET_SSH2_MSG_CHANNEL_OPEN, 'session', self::CHANNEL_KEEP_ALIVE, $this->window_size_server_to_client[self::CHANNEL_KEEP_ALIVE], $packet_size);
        try {
            $this->send_binary_packet($packet);
    ...
    private function get_binary_packet($skip_channel_filter = \false)
    {
        ...
        if (!\is_resource($this->fsock) || \feof($this->fsock)) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\ConnectionClosedException($str);
        }
        $start = \microtime(\true);
        $raw = \stream_get_contents($this->fsock, $this->decrypt_block_size);
        if (!\strlen($raw)) {
            $this->bitmap = 0;
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\ConnectionClosedException('No data received from server');
        }
        if ($this->decrypt) {
            switch ($this->decryptName) {
                ...
                case 'aes256-gcm@openssh.com':
                    ...
                    \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::increment_str($this->decryptInvocationCounter);
                    $this->decrypt->setAAD($temp = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($raw, 4));
                ...
                case 'chacha20-poly1305@openssh.com':
                    ...
                    if (!$this->decrypt instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\ChaCha20) {
                        throw new \LogicException('$this->decrypt is not a ' . \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\ChaCha20::class);
                    }
                    $nonce = \pack('N2', 0, $this->get_seq_no);
                    $this->lengthDecrypt->setNonce($nonce);
                    $temp = $this->lengthDecrypt->decrypt($aad = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($raw, 4));
                    \extract(\unpack('Npacket_length', $temp));
                    /**
                     * @var integer $packet_length
                     */
                    ...
                    break;
                default:
                    if (!$this->hmac_check instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash || !$this->hmac_check_etm) {
                        $raw = $this->decrypt->decrypt($raw);
                        break;
                    }
                    \extract(\unpack('Npacket_length', $temp = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($raw, 4)));
                    /**
                     * @var integer $packet_length
                     */
                    $raw .= $this->read_remaining_bytes($packet_length - $this->decrypt_block_size + 4);
        ...
        \extract(\unpack('Npacket_length/Cpadding_length', \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($raw, 5)));
        /**
         * @var integer $packet_length
        ...
        $payload = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($raw, $packet_length - $padding_length - 1);
        $padding = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($raw, $padding_length);
    ...
    private function read_remaining_bytes($remaining_length)
    {
        ...
        if ($this->decrypt) {
            switch (\true) {
                ...
                case $this->hmac_check instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash && $this->hmac_check_etm:
        ...
        if ($remaining_length < -$this->decrypt_block_size || $remaining_length > 0x9000 || $remaining_length % $this->decrypt_block_size != 0) {
            if (!$this->bad_key_size_fix && self::bad_algorithm_candidate($this->decrypt ? $this->decryptName : '') && !($this->bitmap & \ZOOlanders\YOOessentials\Vendor\phpseclib3\Net\SSH2::MASK_LOGIN)) {
    ...
    private function filter($payload, $skip_channel_filter)
    {
        switch (\ord($payload[0])) {
            case NET_SSH2_MSG_DISCONNECT:
                \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($payload, 1);
                list($reason_code, $message) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('Ns', $payload);
            ...
            case NET_SSH2_MSG_DEBUG:
                \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($payload, 2);
                // second byte is "always_display"
                list($message) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $payload);
        ...
        if ($this->bitmap & self::MASK_CONNECTED && !$this->isAuthenticated() && !\is_bool($payload) && \ord($payload[0]) == NET_SSH2_MSG_USERAUTH_BANNER) {
            \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($payload, 1);
            list($this->banner_message) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $payload);
        ...
        if ($this->bitmap & self::MASK_CONNECTED && $this->isAuthenticated()) {
            ...
            switch (\ord($payload[0])) {
                ...
                case NET_SSH2_MSG_GLOBAL_REQUEST:
                    ...
                    \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($payload, 1);
                    list($request_name) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $payload);
                ...
                case NET_SSH2_MSG_CHANNEL_OPEN:
                    ...
                    \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($payload, 1);
                    list($data, $server_channel) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('sN', $payload);
                    switch ($data) {
                        case 'auth-agent':
                        case 'auth-agent@openssh.com':
                            if (isset($this->agent)) {
                                $new_channel = self::CHANNEL_AGENT_FORWARD;
                                list($remote_window_size, $remote_maximum_packet_size) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $payload);
                ...
                case NET_SSH2_MSG_CHANNEL_WINDOW_ADJUST:
                    \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($payload, 1);
                    list($channel, $window_size) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $payload);
    ...
    protected function get_channel_packet($client_channel, $skip_extended = \false)
    {
        ...
        while (\true) {
            ...
            list($type, $channel) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('CN', $response);
                ...
                switch ($type) {
                    case NET_SSH2_MSG_CHANNEL_EXTENDED_DATA:
                        ...
                        */
                        ...
                        list($data_type_code, $data) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('Ns', $response);
                    ...
                    case NET_SSH2_MSG_CHANNEL_REQUEST:
                        ...
                        list($value) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                        switch ($value) {
                            case 'exit-signal':
                                list(, $signal_name, , $error_message) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('bsbs', $response);
                                ...
                                continue 3;
                            case 'exit-status':
                                list(, $this->exit_status) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('CN', $response);
                    ...
                    case NET_SSH2_MSG_CHANNEL_OPEN:
                        switch ($type) {
                            case NET_SSH2_MSG_CHANNEL_OPEN_CONFIRMATION:
                                list($this->server_channels[$channel], $window_size, $this->packet_size_client_to_server[$channel]) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('NNN', $response);
                                if ($window_size < 0) {
                    ...
                    case NET_SSH2_MSG_CHANNEL_REQUEST:
                        switch ($type) {
                            ...
                            case NET_SSH2_MSG_CHANNEL_FAILURE:
                                return \false;
                            case NET_SSH2_MSG_CHANNEL_DATA:
                                list($data) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                                $this->channel_buffers[$channel][] = \chr($type) . $data;
                                return $this->get_channel_packet($client_channel, $skip_extended);
                            default:
                                $this->disconnect_helper(NET_SSH2_DISCONNECT_BY_APPLICATION);
    ...
    protected function send_binary_packet($data, $logged = null)
    {
        if (!\is_resource($this->fsock) || \feof($this->fsock)) {
            $this->bitmap = 0;
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\ConnectionClosedException('Connection closed prematurely');
        }
        if (!isset($logged)) {
            $logged = $data;
        }
        ...
        switch (\true) {
            ...
            case $this->hmac_create instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash && $this->hmac_create_etm:
                $padding_length += 4;
                $packet_length += 4;
        }
        $padding = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random::string($padding_length);
        // we subtract 4 from packet_length because the packet_length field isn't supposed to include itself
        $packet = \pack('NCa*', $packet_length - 4, $padding_length, $data . $padding);
        $hmac = '';
        if ($this->hmac_create instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash && !$this->hmac_create_etm) {
        ...
        if ($this->encrypt) {
            switch ($this->encryptName) {
                ...
                case 'aes256-gcm@openssh.com':
                    ...
                    \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::increment_str($this->encryptInvocationCounter);
                    $this->encrypt->setAAD($temp = $packet & "\xff\xff\xff\xff");
                    $packet = $temp . $this->encrypt->encrypt(\substr($packet, 4));
                    break;
                case 'chacha20-poly1305@openssh.com':
                    // This should be impossible, but we are checking anyway to narrow the type for Psalm.
                    if (!$this->encrypt instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\ChaCha20) {
                        throw new \LogicException('$this->encrypt is not a ' . \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\ChaCha20::class);
                    }
                    ...
                    break;
                default:
                    $packet = $this->hmac_create instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash && $this->hmac_create_etm ? ($packet & "\xff\xff\xff\xff") . $this->encrypt->encrypt(\substr($packet, 4)) : $this->encrypt->encrypt($packet);
            }
        }
        if ($this->hmac_create instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash && $this->hmac_create_etm) {
            if (($this->hmac_create->getHash() & "\xff\xff\xff\xff") == 'umac') {
    ...
    protected function append_log_helper($constant, $message_number, $message, array &$message_number_log, array &$message_log, &$log_size, &$realtime_log_file, &$realtime_log_wrap, &$realtime_log_size)
    {
        ...
        if (\strlen($message_number) > 2) {
            \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($message);
        }
        switch ($constant) {
    ...
    protected function send_channel_packet($client_channel, $data)
    {
        while (\strlen($data)) {
            ...
            $temp = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($data, $max_size);
            $packet = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CNs', NET_SSH2_MSG_CHANNEL_DATA, $this->server_channels[$client_channel], $temp);
    ...
    {
        if ($this->bitmap & self::MASK_CONNECTED) {
            $data = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('CNss', NET_SSH2_MSG_DISCONNECT, $reason, '', '');
            try {
                $this->send_binary_packet($data);
            } catch (\Exception $e) {
            }
    ...
    protected function format_log(array $message_log, array $message_number_log)
    {
        ...
        for ($i = 0; $i < \count($message_log); $i++) {
            ...
            do {
                ...
                $fragment = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::shift($current_log, $this->log_short_width);
    ...
    public static function getSupportedEncryptionAlgorithms()
    {
        ...
        foreach ($engines as $engine) {
            foreach ($algos as $algo) {
                ...
                if ($obj instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Rijndael) {
    ...
    public function setPreferredAlgorithms(array $methods)
    {
        ...
        foreach ($keys as $key) {
            ...
            if (\count($p) != \count($m)) {
                ...
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException(\implode(', ', $diff) . $msg);
            }
        }
    ...
    public function getServerPublicHostKey()
    {
        ...
        switch ($this->signature_format) {
            ...
            case 'ecdsa-sha2-nistp521':
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC::loadFormat('OpenSSH', $server_public_host_key)->withSignatureFormat('SSH2');
                switch ($this->signature_format) {
                    case 'ssh-ed25519':
                        $hash = 'sha512';
                        break;
                ...
                break;
            case 'ssh-dss':
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA::loadFormat('OpenSSH', $server_public_host_key)->withSignatureFormat('SSH2')->withHash('sha1');
                break;
            case 'ssh-rsa':
            case 'rsa-sha2-256':
            case 'rsa-sha2-512':
                ...
                list(, $signature) = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $signature);
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::loadFormat('OpenSSH', $server_public_host_key)->withPadding(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
                switch ($this->signature_format) {
                    case 'rsa-sha2-512':
                        $hash = 'sha512';
                        break;
            ...
            default:
                $this->disconnect_helper(NET_SSH2_DISCONNECT_HOST_KEY_NOT_VERIFIABLE);
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException('Unsupported signature format');
        }
