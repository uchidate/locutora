 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class KoblitzPrime extends \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime
{
    /**
     * Basis
     *
    ...
    public function multiplyAddPoints(array $points, array $scalars)
    {
        ...
        if (!isset($this->basis)) {
            $factory = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\PrimeField($this->order);
            $tempOne = $factory->newInteger($one);
            $tempTwo = $factory->newInteger($two);
            $tempThree = $factory->newInteger(new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(3));
     ...
     * @return BigInteger[]
     */
    protected static function extendedGCD(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger $u, \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger $v)
    {
        $one = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(1);
