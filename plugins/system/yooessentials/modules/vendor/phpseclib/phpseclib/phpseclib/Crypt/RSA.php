...
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\PrivateKey;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class RSA extends \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\AsymmetricKey
{
    /**
     * Algorithm Name
     *
    ...
    public static function createKey($bits = 2048)
    {
        ...
        if ($num_primes == 2 && $bits >= 384 && self::$defaultExponent == 65537) {
            ...
            if (self::$engines['OpenSSL']) {
                ...
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::load($privatekeystr);
            }
        }
        static $e;
        if (!isset($e)) {
            $e = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(self::$defaultExponent);
        }
        $n = clone self::$one;
        ...
        do {
            for ($i = 1; $i <= $num_primes; $i++) {
                if ($i != $num_primes) {
                    $primes[$i] = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger::randomPrime($regSize);
                } else {
                    \extract(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger::minMaxBits($bits));
                    /** @var BigInteger $min
                     *  @var BigInteger $max
                     */
                    ...
                    $primes[$i] = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger::randomRangePrime($min, $max);
                }
        ...
        // }
        $privatekey = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\PrivateKey();
    ...
    {
        $key = $components['isPublicKey'] ? new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\PublicKey() : new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\PrivateKey();
        ...
        if ($components['format'] == \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS::class) {
    ...
    protected function __construct()
    {
        ...
        $this->mgfHash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha256');
    ...
    {
        return new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger($x, 256);
    }
    /**
    ...
    protected function emsa_pkcs1_v1_5_encode_without_null($m, $emLen)
    {
        ...
        switch ($this->hash->getHash()) {
            ...
            case 'sha512/256':
                ...
                break;
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('md2 and md5 require NULLs');
        }
        $t .= $h;
        $tLen = \strlen($t);
        if ($emLen < $tLen + 11) {
    ...
    public function withHash($hash)
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha512/224':
            case 'sha512/256':
                $new->hash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash($hash);
                break;
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms are: md2, md5, sha1, sha256, sha384, sha512, sha224, sha512/224, sha512/256');
        }
    ...
    public function withMGFHash($hash)
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha512/224':
            case 'sha512/256':
                $new->mgfHash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash($hash);
                break;
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms are: md2, md5, sha1, sha256, sha384, sha512, sha224, sha512/224, sha512/256');
        }
    ...
    public function withPadding($padding)
    {
        ...
        if ($numSelected > 1) {
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\InconsistentSetupException('Multiple encryption padding modes have been selected; at most only one should be selected');
        }
        ...
        if ($numSelected > 1) {
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\InconsistentSetupException('Multiple signature padding modes have been selected; at most only one should be selected');
        }
