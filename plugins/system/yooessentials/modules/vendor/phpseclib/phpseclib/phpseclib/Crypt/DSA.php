...
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\Parameters;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\PrivateKey;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\PublicKey;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class DSA extends \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\AsymmetricKey
{
    /**
     * Algorithm Name
     *
    ...
    public static function createParameters($L = 2048, $N = 224)
    {
        ...
        $two = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(2);
        $q = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger::randomPrime($N);
        $divisor = $q->multiply($two);
        do {
            $x = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger::random($L);
        ...
        $dsa = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\Parameters();
        $dsa->p = $p;
        $dsa->q = $q;
        $dsa->g = $g;
        return $dsa;
    ...
    public static function createKey(...$args)
    {
        ...
        } elseif (\count($args) == 1 && $args[0] instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\Parameters) {
            $params = $args[0];
        } elseif (!\count($args)) {
            $params = self::createParameters();
        } else {
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\InsufficientSetupException('Valid parameters are either two integers (L and N), a single DSA object or no parameters at all.');
        }
        $private = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\PrivateKey();
        $private->p = $params->p;
        $private->q = $params->q;
        $private->g = $params->g;
        $private->x = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger::randomRange(self::$one, $private->q->subtract(self::$one));
    ...
    protected static function onLoad(array $components)
    {
        ...
        if (!isset($components['x']) && !isset($components['y'])) {
            $new = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\Parameters();
        } elseif (isset($components['x'])) {
            $new = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\PrivateKey();
            $new->x = $components['x'];
        } else {
            $new = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA\PublicKey();
        }
        $new->p = $components['p'];
        $new->q = $components['q'];
        $new->g = $components['g'];
    ...
    public function getParameters()
    {
        ...
        return \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA::load($key, 'PKCS1')->withHash($this->hash->getHash())->withSignatureFormat($this->shortFormat);
    }
    /**
