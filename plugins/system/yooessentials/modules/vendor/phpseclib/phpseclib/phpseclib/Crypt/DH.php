...
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\Parameters;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\PrivateKey;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\PublicKey;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class DH extends \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\AsymmetricKey
{
    /**
     * Algorithm Name
     *
    ...
    {
        $params = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\Parameters();
        if (\count($args) == 2 && $args[0] instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger && $args[1] instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger) {
            //if (!$args[0]->isPrime()) {
            ...
            $params->prime = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger::randomPrime($args[0]);
            $params->base = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(2);
            return $params;
        ...
        $params->prime = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger($prime, 16);
        $params->base = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(2);
        return $params;
    }
    /**
     ...
     */
    public static function createKey(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\Parameters $params, $length = 0)
    {
        ...
        $key = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\PrivateKey();
        $key->prime = $params->prime;
        $key->base = $params->base;
        $key->privateKey = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger::randomRange($one, $max);
    ...
    {
        if ($private instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\PrivateKey) {
            // DH\PrivateKey
            switch (\true) {
                case $public instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\PublicKey:
                ...
                case \is_string($public):
                    $public = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger($public, -256);
                // fall-through
                case $public instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger:
        ...
        if ($private instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC\PrivateKey) {
            switch (\true) {
                case $public instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC\PublicKey:
    ...
    protected static function onLoad(array $components)
    {
        if (!isset($components['privateKey']) && !isset($components['publicKey'])) {
            $new = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\Parameters();
        } else {
            $new = isset($components['privateKey']) ? new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\PrivateKey() : new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DH\PublicKey();
        }
    ...
    {
        throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedOperationException('DH does not use a hash algorithm');
    }
    /**
