 ...
 */
namespace ZOOlanders\YOOessentials\Vendor\phpseclib3\File;
...
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\PublicKeyLoader;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException;
use ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element;
...
class X509
{
    ...
    public function __construct()
    {
        ...
        if (!self::$oidsLoaded) {
            ...
            \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::loadOIDs([
    ...
    public function loadX509($cert, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcert = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::extractBER($cert);
        ...
        $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($cert);
        if ($decoded) {
            $x509 = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\Certificate::MAP);
        }
        ...
        $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($key, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\SubjectPublicKeyInfo::MAP);
    ...
    public function saveX509(array $cert, $format = self::FORMAT_PEM)
    {
        ...
        switch (\true) {
            ...
            case \is_object($cert['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey']):
                break;
            default:
                $cert['tbsCertificate']['subjectPublicKeyInfo'] = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element(\base64_decode(\preg_replace('#-.+-|[\\r\\n]#', '', $cert['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey'])));
        }
        ...
        $filters = [];
        $type_utf8_string = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        ...
        /* in the case of policyQualifiers/qualifier, the type has to be \phpseclib3\File\ASN1::TYPE_IA5_STRING.
             ...
             characters.
           */
        $filters['policyQualifiers']['qualifier'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_IA5_STRING];
        \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $cert = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($cert, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\Certificate::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $cert;
            // case self::FORMAT_PEM:
            default:
                return "-----BEGIN CERTIFICATE-----\r\n" . \chunk_split(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($cert), 64) . '-----END CERTIFICATE-----';
        }
    }
    /**
        ...
        if ($extensions) {
            for ($i = 0; $i < \count($extensions); $i++) {
                ...
                if (!\is_bool($map)) {
                    ...
                    $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($value);
                    if (!$decoded) {
                        continue;
                    }
                    $mapped = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], $map, ['iPAddress' => $decoder]);
                    ...
                    if ($id == 'id-ce-certificatePolicies') {
                        for ($j = 0; $j < \count($value); $j++) {
                            ...
                            for ($k = 0; $k < \count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                if ($map !== \false) {
                                    $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($subvalue);
                                    if (!$decoded) {
                                        continue;
                                    }
                                    $mapped = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
    ...
    private function mapOutExtensions(array &$root, $path)
    {
        ...
        if (\is_array($extensions)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                if ($extensions[$i] instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element) {
                    continue;
                }
                $id = $extensions[$i]['extnId'];
                $value =& $extensions[$i]['extnValue'];
                        ...
                        for ($j = 0; $j < \count($value); $j++) {
                            ...
                            for ($k = 0; $k < \count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                if ($map !== \false) {
                                    ...
                                    $subvalue = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element(\ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($subvalue, $map));
                                }
                            }
                        }
                        break;
                    case 'id-ce-authorityKeyIdentifier':
                        ...
                        if (isset($value['authorityCertSerialNumber'])) {
                            if ($value['authorityCertSerialNumber']->toBytes() == '') {
                                $temp = \chr(\ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::CLASS_CONTEXT_SPECIFIC << 6 | 2) . "\x01\x00";
                                $value['authorityCertSerialNumber'] = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element($temp);
                            }
                        }
                }
                /* [extnValue] contains the DER encoding of an ASN.1 value
                ...
                if (\is_bool($map)) {
                    ...
                    $value = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($value, $map, ['iPAddress' => [static::class, 'encodeIP']]);
                }
            }
        }
    }
            ...
            for ($i = 0; $i < \count($attributes); $i++) {
                ...
                if (\is_array($attributes[$i]['value'])) {
                    ...
                    for ($j = 0; $j < \count($values); $j++) {
                        $value = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($values[$j], \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\AttributeValue::MAP);
                        $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($value);
                        if (!\is_bool($map)) {
                            if (!$decoded) {
                                continue;
            ...
            for ($i = 0; $i < $size; $i++) {
                ...
                if ($map === \false) {
                    ...
                    for ($j = 0; $j < \count($values); $j++) {
                        ...
                        if (!\is_bool($map)) {
                            $temp = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($values[$j], $map);
                            $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($temp);
                            if (!$decoded) {
                                continue;
                            }
                            $values[$j] = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\AttributeValue::MAP);
                        }
                    }
                }
            }
        ...
        if (\is_array($dns)) {
            for ($i = 0; $i < \count($dns); $i++) {
                for ($j = 0; $j < \count($dns[$i]); $j++) {
                    $type = $dns[$i][$j]['type'];
                    $value =& $dns[$i][$j]['value'];
                    if (\is_object($value) && $value instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element) {
                        ...
                        if (!\is_bool($map)) {
                            ...
                            $value = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                        }
                    }
                }
            }
        ...
        if (\is_array($dns)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                for ($j = 0; $j < \count($dns[$i]); $j++) {
                    ...
                    $map = $this->getMapping($type);
                    if (!\is_bool($map)) {
                        $value = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element(\ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($value, $map));
                    }
                }
            }
        }
    ...
    private function getMapping($extnId)
    {
        ...
        switch ($extnId) {
            case 'id-ce-keyUsage':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\KeyUsage::MAP;
            case 'id-ce-basicConstraints':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\BasicConstraints::MAP;
            case 'id-ce-subjectKeyIdentifier':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\KeyIdentifier::MAP;
            case 'id-ce-cRLDistributionPoints':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CRLDistributionPoints::MAP;
            case 'id-ce-authorityKeyIdentifier':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\AuthorityKeyIdentifier::MAP;
            case 'id-ce-certificatePolicies':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CertificatePolicies::MAP;
            case 'id-ce-extKeyUsage':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\ExtKeyUsageSyntax::MAP;
            case 'id-pe-authorityInfoAccess':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\AuthorityInfoAccessSyntax::MAP;
            case 'id-ce-subjectAltName':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\SubjectAltName::MAP;
            case 'id-ce-subjectDirectoryAttributes':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\SubjectDirectoryAttributes::MAP;
            case 'id-ce-privateKeyUsagePeriod':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\PrivateKeyUsagePeriod::MAP;
            case 'id-ce-issuerAltName':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\IssuerAltName::MAP;
            case 'id-ce-policyMappings':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\PolicyMappings::MAP;
            case 'id-ce-nameConstraints':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\NameConstraints::MAP;
            case 'netscape-cert-type':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\netscape_cert_type::MAP;
            ...
            case 'id-qt-unotice':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\UserNotice::MAP;
            ...
            case 'pkcs-9-at-unstructuredName':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\PKCS9String::MAP;
            case 'pkcs-9-at-challengePassword':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\DirectoryString::MAP;
            case 'pkcs-9-at-extensionRequest':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\Extensions::MAP;
            // CRL extensions.
            case 'id-ce-cRLNumber':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CRLNumber::MAP;
            case 'id-ce-deltaCRLIndicator':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CRLNumber::MAP;
            case 'id-ce-issuingDistributionPoint':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\IssuingDistributionPoint::MAP;
            case 'id-ce-freshestCRL':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CRLDistributionPoints::MAP;
            case 'id-ce-cRLReasons':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CRLReason::MAP;
            case 'id-ce-invalidityDate':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\InvalidityDate::MAP;
            case 'id-ce-certificateIssuer':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CertificateIssuer::MAP;
            case 'id-ce-holdInstructionCode':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\HoldInstructionCode::MAP;
            case 'id-at-postalAddress':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\PostalAddress::MAP;
        }
        return \false;
    }
    /**
    ...
    private function validateSignatureHelper($publicKeyAlgorithm, $publicKey, $signatureAlgorithm, $signature, $signatureSubject)
    {
        switch ($publicKeyAlgorithm) {
            case 'id-RSASSA-PSS':
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::loadFormat('PSS', $publicKey);
                break;
            case 'rsaEncryption':
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::loadFormat('PKCS8', $publicKey);
                switch ($signatureAlgorithm) {
                    ...
                    case 'sha512WithRSAEncryption':
                        $key = $key->withHash(\preg_replace('#WithRSAEncryption$#', '', $signatureAlgorithm))->withPadding(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
                        break;
                    default:
                        throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('Signature algorithm unsupported');
                }
                break;
            case 'id-Ed25519':
            case 'id-Ed448':
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC::loadFormat('PKCS8', $publicKey);
                break;
            case 'id-ecPublicKey':
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC::loadFormat('PKCS8', $publicKey);
                switch ($signatureAlgorithm) {
                ...
                break;
            case 'id-dsa':
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA::loadFormat('PKCS8', $publicKey);
                switch ($signatureAlgorithm) {
                ...
                break;
            default:
                throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('Public key algorithm unsupported');
        }
    ...
    public function getDNProp($propName, array $dn = null, $withType = \false)
    {
        ...
        $filters = [];
        $filters['value'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::setFilters($filters);
        ...
        for ($i = 0; $i < \count($dn); $i++) {
            if ($dn[$i][0]['type'] == $propName) {
                ...
                if (!$withType) {
                    if (\is_array($v)) {
                        foreach ($v as $type => $s) {
                            $type = \array_search($type, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::ANY_MAP);
                            if ($type !== \false && \array_key_exists($type, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::STRING_TYPE_SIZE)) {
                                $s = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::convert($s, $type);
                                if ($s !== \false) {
                                    $v = $s;
                                    break;
                                }
                    ...
                    } elseif (\is_object($v) && $v instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element) {
                        $map = $this->getMapping($propName);
                        if (!\is_bool($map)) {
                            $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($v);
                            if (!$decoded) {
                                return \false;
                            }
                            $v = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                        }
                    }
                }
                $result[] = $v;
    ...
    public function getDN($format = self::DN_ARRAY, array $dn = null)
    {
        ...
        switch ((int) $format) {
            ...
            case self::DN_ASN1:
                $filters = [];
                $filters['rdnSequence']['value'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
                \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::setFilters($filters);
                $this->mapOutDNs($dn, 'rdnSequence');
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($dn, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\Name::MAP);
            case self::DN_CANON:
                ...
                foreach ($dn['rdnSequence'] as $rdn) {
                    foreach ($rdn as $i => $attr) {
                        ...
                        if (\is_array($attr['value'])) {
                            foreach ($attr['value'] as $type => $v) {
                                $type = \array_search($type, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::ANY_MAP, \true);
                                if ($type !== \false && \array_key_exists($type, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::STRING_TYPE_SIZE)) {
                                    $v = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::convert($v, $type);
                                    if ($v !== \false) {
                                        $v = \preg_replace('/\\s+/', ' ', $v);
                    ...
                    $result .= \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($rdn, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\RelativeDistinguishedName::MAP);
                }
                return $result;
            case self::DN_HASH:
                $dn = $this->getDN(self::DN_CANON, $dn);
                $hash = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Hash('sha1');
                $hash = $hash->hash($dn);
                \extract(\unpack('Vhash', $hash));
                return \strtolower(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::bin2hex(\pack('N', $hash)));
        }
        ...
        foreach ($dn['rdnSequence'] as $field) {
            ...
            } elseif (\is_object($value) && $value instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element) {
                $callback = function ($x) {
    ...
    public function getChain()
    {
        ...
        foreach ($chain as $key => $value) {
            $chain[$key] = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\X509();
     ...
     * @return void
     */
    public function setPublicKey(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PublicKey $key)
    {
        $this->publicKey = $key;
    }
    /**
     ...
     */
    public function setPrivateKey(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PrivateKey $key)
    {
        $this->privateKey = $key;
    }
    /**
    ...
    public function getPublicKey()
    {
        ...
        switch ($keyinfo['algorithm']['algorithm']) {
            case 'id-RSASSA-PSS':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::loadFormat('PSS', $key);
            case 'rsaEncryption':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::loadFormat('PKCS8', $key)->withPadding(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
            case 'id-ecPublicKey':
            case 'id-Ed25519':
            case 'id-Ed448':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC::loadFormat('PKCS8', $key);
            case 'id-dsa':
                return \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA::loadFormat('PKCS8', $key);
        }
        return \false;
    }
    /**
    ...
    public function loadCSR($csr, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcsr = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::extractBER($csr);
        ...
        $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($csr);
        if (!$decoded) {
            $this->currentCert = \false;
            return \false;
        }
        $csr = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CertificationRequest::MAP);
    ...
    public function saveCSR(array $csr, $format = self::FORMAT_PEM)
    {
        ...
        switch (\true) {
            ...
            case \is_object($csr['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey']):
                break;
            default:
                $csr['certificationRequestInfo']['subjectPKInfo'] = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element(\base64_decode(\preg_replace('#-.+-|[\\r\\n]#', '', $csr['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey'])));
        }
        $filters = [];
        $filters['certificationRequestInfo']['subject']['rdnSequence']['value'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $csr = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($csr, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CertificationRequest::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $csr;
            // case self::FORMAT_PEM:
            default:
                return "-----BEGIN CERTIFICATE REQUEST-----\r\n" . \chunk_split(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($csr), 64) . '-----END CERTIFICATE REQUEST-----';
        }
    }
    /**
     * Load a SPKAC CSR
    ...
    public function loadSPKAC($spkac)
    {
        ...
        $temp = \preg_match('#^[a-zA-Z\\d/+]*={0,2}$#', $temp) ? \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_decode($temp) : \false;
        if ($temp != \false) {
            $spkac = $temp;
        }
        $orig = $spkac;
        ...
        $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($spkac);
        if (!$decoded) {
            $this->currentCert = \false;
            return \false;
        }
        $spkac = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\SignedPublicKeyAndChallenge::MAP);
    ...
    public function saveSPKAC(array $spkac, $format = self::FORMAT_PEM)
    {
        ...
        switch (\true) {
            ...
            case \is_object($spkac['publicKeyAndChallenge']['spki']['subjectPublicKey']):
                break;
            default:
                $spkac['publicKeyAndChallenge']['spki'] = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element(\base64_decode(\preg_replace('#-.+-|[\\r\\n]#', '', $spkac['publicKeyAndChallenge']['spki']['subjectPublicKey'])));
        }
        $spkac = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($spkac, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\SignedPublicKeyAndChallenge::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $spkac;
            // case self::FORMAT_PEM:
            default:
                ...
                return 'SPKAC=' . \ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($spkac);
        }
    }
    /**
    ...
    public function loadCRL($crl, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcrl = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::extractBER($crl);
        ...
        $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($crl);
        if (!$decoded) {
            $this->currentCert = \false;
            return \false;
        }
        $crl = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CertificateList::MAP);
    ...
    public function saveCRL(array $crl, $format = self::FORMAT_PEM)
    {
        ...
        $filters = [];
        $filters['tbsCertList']['issuer']['rdnSequence']['value'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        $filters['tbsCertList']['signature']['parameters'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        $filters['signatureAlgorithm']['parameters'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        if (empty($crl['tbsCertList']['signature']['parameters'])) {
            $filters['tbsCertList']['signature']['parameters'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_NULL];
        }
        if (empty($crl['signatureAlgorithm']['parameters'])) {
            $filters['signatureAlgorithm']['parameters'] = ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_NULL];
        }
        \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $crl = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeDER($crl, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\CertificateList::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $crl;
            // case self::FORMAT_PEM:
            default:
                return "-----BEGIN X509 CRL-----\r\n" . \chunk_split(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($crl), 64) . '-----END X509 CRL-----';
        }
    }
    /**
    ...
    {
        if ($date instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element) {
            return $date;
        }
     ...
     * @return mixed
     */
    public function sign(\ZOOlanders\YOOessentials\Vendor\phpseclib3\File\X509 $issuer, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\X509 $subject)
    {
        ...
        if ($signatureAlgorithm != 'id-RSASSA-PSS') {
            ...
            $r = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS::load($issuer->privateKey->withPassword()->toString('PSS'));
            $signatureAlgorithm = ['algorithm' => 'id-RSASSA-PSS', 'parameters' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS::savePSSParams($r)];
        }
        if (isset($subject->currentCert) && \is_array($subject->currentCert) && isset($subject->currentCert['tbsCertificate'])) {
            ...
            /* "The serial number MUST be a positive integer"
                        ...
                        */
            $serialNumber = !empty($this->serialNumber) ? $this->serialNumber : new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Random::string(20) & "" . \str_repeat("\xff", 19), 256);
     ...
     * @return mixed
     */
    public function signCRL(\ZOOlanders\YOOessentials\Vendor\phpseclib3\File\X509 $issuer, \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\X509 $crl)
    {
        if (!\is_object($issuer->privateKey) || empty($issuer->dn)) {
            return \false;
        }
        ...
        if (!empty($this->serialNumber)) {
            ...
            $crlNumber = $crlNumber !== \false ? $crlNumber->add(new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger(1)) : null;
        }
        $this->removeExtension('id-ce-authorityKeyIdentifier');
     ...
     * @return string
     */
    private static function identifySignatureAlgorithm(\ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\Common\PrivateKey $key)
    {
        if ($key instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA) {
            if ($key->getPadding() & \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::SIGNATURE_PSS) {
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms for RSA are: md2, md5, sha1, sha224, sha256, sha384, sha512');
        }
        if ($key instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\DSA) {
            switch ($key->getHash()) {
                case 'sha1':
                case 'sha224':
                case 'sha256':
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms for DSA are: sha1, sha224, sha256');
        }
        if ($key instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\EC) {
            switch ($key->getCurve()) {
                case 'Ed25519':
                case 'Ed448':
            ...
            throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms for EC are: sha1, sha224, sha256, sha384, sha512');
        }
        throw new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported public key classes are: RSA, DSA, EC');
    }
    /**
    ...
    public function setEndDate($date)
    {
        ...
        */
        if (\is_string($date) && \strtolower($date) === 'lifetime') {
            ...
            $temp = \chr(\ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_GENERALIZED_TIME) . \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::encodeLength(\strlen($temp)) . $temp;
            $this->endDate = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element($temp);
        } else {
            if (!\is_object($date) || !$date instanceof \DateTimeInterface) {
                $date = new \DateTimeImmutable($date, new \DateTimeZone(@\date_default_timezone_get()));
            }
    ...
    {
        $this->serialNumber = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger($serial, $base);
    }
    /**
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (\true) {
            ...
            case !\is_object($key):
                return \false;
            case $key instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Element:
                // Assume the element is a bitstring-packed key.
                $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($key->element);
                if (!$decoded) {
                    return \false;
                }
                $raw = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], ['type' => \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::TYPE_BIT_STRING]);
                if (empty($raw)) {
                    return \false;
                }
                // If the key is private, compute identifier from its corresponding public key.
                $key = \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\PublicKeyLoader::load($raw);
                ...
                // Is a public key.
                break;
            case $key instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\X509:
    ...
    {
        $format = $this->publicKey instanceof \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA && $this->publicKey->getPadding() & \ZOOlanders\YOOessentials\Vendor\phpseclib3\Crypt\RSA::SIGNATURE_PSS ? 'PSS' : 'PKCS8';
        $publicKey = \base64_decode(\preg_replace('#-.+-|[\\r\\n]#', '', $this->publicKey->toString($format)));
        $decoded = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::decodeBER($publicKey);
        if (!$decoded) {
            return \false;
        }
        $mapped = \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \ZOOlanders\YOOessentials\Vendor\phpseclib3\File\ASN1\Maps\SubjectPublicKeyInfo::MAP);
        if (!\is_array($mapped)) {
            return \false;
        }
    ...
    {
        $serial = new \ZOOlanders\YOOessentials\Vendor\phpseclib3\Math\BigInteger($serial);
