...
use ZOOlanders\YOOessentials\Sources\Csv\CsvResolver;
use ZOOlanders\YOOessentials\Sources\Csv\CsvSource;
use ZOOlanders\YOOessentials\Util\Arr;

class CsvQueryType extends AbstractQueryType implements HasSourceInterface
{
            ...
            'fields' => [
                ...
                $this->name() => [
                    ...
                    'args' => [
                        ...
                        'filters' => [
                            'type' => ['listOf' => DynamicSourceInputType::nameForInputType(CsvFilterType::TYPE_NAME)],
                            'defaultValue' => []
                        ],
                        'ordering' => [
                            'type' => ['listOf' => DynamicSourceInputType::nameForInputType(CsvOrderingType::TYPE_NAME)],
                            'defaultValue' => []
                        ],
                        'cache' => [
                            'type' => 'Int'
    ...
    public static function resolve($root, array $args): array
    {
        ...
        try {
            return self::resolveFromCache($source, $args, function () use ($source, $args, $root) {
                return (new CsvResolver($source, $args, $root))->resolve();
            });
        } catch (\Exception $e) {
            Event::emit('yooessentials.error', [
                'addon' => 'source',
                'action' => 'source-csv-resolve',
                'args' => $args,
                'error' => $e->getMessage(),
                'exception' => $e
            ]);
    ...
    protected function getHeaders(): array
    {
        ...
        foreach ($this->source->csv()->getHeader() as $header) {
            ...
            $name = Str::titleCase(utf8_encode($header ?? $id)); // make sure to return no empty string utf-8 encoded
