Locutora.com - restauração
=========================

1. Copie `configuration.example.php` para `configuration.php`.
2. Preencha a senha do banco e gere um novo valor aleatório para `$secret`.
3. Confirme host, banco, usuário e prefixo para o ambiente de destino.
4. Renomeie `htaccess.` para `.htaccess` ao publicar em Apache.
5. Restaure o banco usando um backup criptografado separado. Dumps de produção
   não entram no Git porque podem conter usuários, formulários e credenciais.
